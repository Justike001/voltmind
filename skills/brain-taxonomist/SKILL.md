...
description: |
  ...
  via `voltmind schema show --json` — no hardcoded directory table. Also
  runs periodic taxonomy drift detection via `voltmind schema review-orphans`.
triggers:
...
driven by `voltmind schema show --json`. This means:
- A user who runs `voltmind schema use voltmind-recommended` gets the full
  ...
  plus all voltmind-base types).
- A user who authored a custom pack via `voltmind schema init` + edit gets
  filing recommendations based on THEIR taxonomy, not voltmind's defaults.
...
```bash
voltmind schema show --json | jq '.page_types[] | select(.primitive == "entity")'
```
...
The `voltmind-recommended` pack treats books as `media/books/<category>/<slug>.md`
...
walk it from `voltmind schema show --json` instead of hardcoding here.
...
voltmind schema review-orphans --json

# What's the overall health?
voltmind doctor --json | jq '.checks[] | select(.name == "schema_pack_consistency")'
```
...
**Suggested next step:** `voltmind schema detect --source <source_id>` then
`voltmind schema review-candidates`.
```

## Anti-Patterns

- **Hardcoded directory table in this skill.** Every decision goes through
  `voltmind schema show --json`. v0.39+ broke the old hardcoded table on
  purpose so users on `voltmind-recommended` or custom packs get the right
...
- **Auto-applying a `voltmind schema review-candidates --apply` decision.**
...
- **Never hardcode a directory table in this skill.** Every decision goes
  through `voltmind schema show --json`. The active pack is canonical.
...
- Hardcoded directory table REMOVED. Every decision now reads the active
  schema pack via `voltmind schema show --json`. Single source of truth.
- Book taxonomy moved from skill-text to the `voltmind-recommended` pack's
  media/books/ branch (see `src/core/schema-pack/base/voltmind-recommended.yaml`).
