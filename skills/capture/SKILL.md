...
fragment, or any text into their brain, run `voltmind capture`. Don't reach
for `voltmind put` or commit-then-sync — `capture` is the front door and it
...
`--quiet` prints only the slug (use for `SLUG=$(voltmind capture "..." --quiet)`).
`--json` prints structured output for downstream tools.

## Anti-Patterns

- **Don't reach for `voltmind put`.** That's the old per-page primitive that
...
- **Don't try to bulk-import dozens of files by looping over `voltmind capture`.**
  That's what `voltmind sync` (or `voltmind import`) is for. Capture is for
...
- Bulk ingestion of many files → `skills/media-ingest/SKILL.md` or `voltmind sync` instead
