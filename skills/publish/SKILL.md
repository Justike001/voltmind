...
This is a **code + skill pair**: the deterministic code (`voltmind publish`) does
...
voltmind publish brain/companies/acme.md

# Password protected (auto-generate password)
voltmind publish brain/companies/acme.md --password

# Password protected (specific password)
voltmind publish brain/companies/acme.md --password "secret123"

# Custom title
voltmind publish brain/companies/acme.md --password --title "Acme -- Deal Analysis"

# Custom output path
voltmind publish brain/companies/acme.md --out /tmp/acme-share.html
```
...
```bash
voltmind publish brain/people/jane-doe.md --password --out ~/Desktop/jane-briefing.html
```
...
voltmind publish brain/companies/acme.md --password "secret" --out /tmp/acme.html

# Upload to Supabase Storage
voltmind files upload /tmp/acme.html --page shares/acme

# Get a signed URL (1-hour expiry)
voltmind files signed-url shares/acme/acme.html
```
...
```bash
voltmind publish brain/trips/japan-2026.md --out trip.html
...
```bash
voltmind publish brain/companies/acme.md --password "same-password" --out shares/acme.html
```
...
- **Assuming the user wants raw markdown.** The publish command produces beautiful HTML. Don't copy-paste markdown when `voltmind publish` exists.
...
- `voltmind publish` -- deterministic HTML generation (no LLM calls)
- `voltmind files upload` -- upload to cloud storage (optional)
- `voltmind files signed-url` -- generate access links (optional)
