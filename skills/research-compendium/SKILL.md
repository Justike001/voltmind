...
via `voltmind recall` and search. An injected instruction archived today
...
- Binary originals (PDFs, images) attach to their source page via
  `voltmind files upload-raw <file> --page research/<topic-slug>/sources/NN-<source-slug>`.
- Wire links in both directions as you go: `voltmind link` each source ↔ index
  and summary ↔ compendium, and run `voltmind check-backlinks check` at close-out.
...
`archived`/`summarized`/`mirrored` booleans, each `voltmind lsd` pass (seed
...
| `compendium+` | Grounded | Full-text primaries (papers/RFCs/primary blog posts) acquired and each summary re-read *against the driving question* (mechanism + tension, not generic recap). One formal `voltmind lsd` pass. Claims ledger + fact-check gate turn ON. | moderate |
| `compendium++` | Deep | Books enter (summary-tier, with verbatim quotes). **Counter-canon hunt**: acquire the best critiques/recantations of each pillar. 2-3 `voltmind lsd` passes, cross-modal eval per pass. | higher |
| `compendium+++` | Saturated | Full book-mirrors on the 1-2 most central books (via [book-mirror](../book-mirror/SKILL.md), user opt-in). Cross-axis mapping as its own section. `voltmind lsd` repeated until new passes stop surfacing survivors (log the saturation point). | high |
...
- **`voltmind lsd` passes are real runs** on the archived corpus (not in-head
  synthesis): `voltmind lsd "<the driving question>" --save --max-cost 5`. Seed
...
Brain first: `voltmind query "<topic>"` and `voltmind search <terms>` — the brain
...
hooks are what make later `voltmind lsd` passes productive — pre-wired
...
[cross-modal-review](../cross-modal-review/SKILL.md) (or `voltmind eval
...
shareable HTML page with `voltmind publish`. Run the fact-check gate BEFORE
...
- Output written under the directories listed in `writes_to:`; links wired in
  both directions and validated with `voltmind check-backlinks check`.
...
itself when routed — nothing in the voltmind runtime mechanically blocks an
