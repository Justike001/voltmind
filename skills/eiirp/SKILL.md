...
# pack via `voltmind schema show --json`. List the voltmind-recommended set
...
voltmind schema review-candidates --json
voltmind schema review-candidates --apply <prefix-or-type-name>
```

**Confidence floor (codex finding #9):** when `voltmind schema suggest`
...
- The schema-pack engine writes the delta to
  `~/.voltmind/schema-pack-deltas/` — review and merge into the active
  pack via `voltmind schema edit` (or hand-edit the YAML).
...
- Check if a brain page exists (`voltmind search "<name>"` or `voltmind get_page people/<slug>`).
- If exists: update State, append Timeline entry citing this research.
- If not: create with enrichment.

### 4c. Commit and verify
After ALL pages are written, run `voltmind sync` (or commit + push in the
...
```bash
voltmind check-resolvable                         # routing-table reachability
voltmind doctor --json                            # health surface
voltmind search "<topic keywords>"                # brain pages findable
voltmind orphans                                  # any pages without inbound links?
```
...
- [ ] `voltmind doctor` reports `schema_pack_consistency: ok`
...
- **Hardcoding directory tables in EIIRP's logic.** Every filing decision
  reads `voltmind schema show --json`. Users on `voltmind-recommended` AND
...
- **Auto-applying low-confidence schema suggestions.** Confidence < 0.6
  from `voltmind schema suggest` is "manual review required" per codex
...
- **EIIRP consumes the active schema pack as data.** Never hard-code directory tables in EIIRP's logic — read from `voltmind schema show --json` so users who picked `voltmind-recommended` OR custom packs get the right behavior automatically.

## Changelog

### v1.0.0 — voltmind v0.39.0.0
...
- Phase 3 SCHEMA CHECK rewritten to consume the v0.39 cathedral CLI
  ...
  `brain/schema.md`.
- Phase 5 SKILL GRAPH AUDIT calls `voltmind check-resolvable` instead of
  upstream `scripts/skill-dry-check.mjs`.
- Phase 6 verification uses `voltmind doctor`'s schema_pack_consistency
