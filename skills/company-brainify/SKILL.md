...
# Brain-first in its native form: Phase-1 discovery runs through voltmind
...
> The grep pipelines below TRIAGE; `voltmind query` finds what keyword patterns miss.
>
...
pre-sanitization version, and voltmind takes/facts carry evaluative claims
...
| **Takes/facts** | Any take or fact referencing the above categories — performance, comp, retention, weakness, management risk. Fact rows are DELETED from the page's Facts fence, never merely expired with `voltmind forget` |
...
- **Takes** — evaluative claims in page takes fences (`voltmind takes search`)
- **Facts** — hot-memory facts (`voltmind recall --grep`)
- **Back-links** — after edits, `voltmind check-backlinks check` confirms no page
...
```bash
BRAIN="$(voltmind config get sync.repo_path)"
cd "$BRAIN"
```
...
1. Retrieval discovery — hybrid search catches judgment-shaped content that no
   ...
   ```bash
   voltmind query "compensation, equity, or salary discussions about team members" --limit 50
   voltmind query "performance concerns, underperformance, or who is struggling" --limit 50
   voltmind query "considering leaving, retention conversations, departure rumors" --limit 50
   voltmind takes search "performance" --limit 50
   voltmind recall --grep "salary"
   ```
...
- **Standing up a NEW team brain (default, preferred):** sanitize a STAGING
  ...
  STAGING="$HOME/.voltmind/backups/brainify-staging-$(date +%Y%m%d-%H%M%S)"
...
**Facts: `forget` is NOT removal.** `voltmind forget <fact-id>` expires a fact
...
(`voltmind recall --grep`), then delete the row from the page's Facts fence
(step 5), exactly like a sensitive take. On an in-place shared brain, the
page edit must then be re-synced (`voltmind sync` re-imports the edited page)
...
(`voltmind sweep`, or wait for the serve-resident sweep), then confirm with
`voltmind recall --grep` that the row is actually gone. An edited page over
...
**in-place shared-brain** path, run `voltmind sync` so the page content matches
the markdown, then reconcile and verify the facts index as above. Either way,
run `voltmind check-backlinks check` to catch pages still pointing at removed
content.
...
```bash
voltmind query "what is alice-example's compensation" --limit 10
voltmind query "who is underperforming or at risk of leaving" --limit 10
voltmind takes search "weakness" --limit 20
```
...
```bash
PERSONAL="$(voltmind config get sync.repo_path)"
mkdir -p "$HOME/.voltmind/backups" && chmod 700 "$HOME/.voltmind/backups"
WORK="$HOME/.voltmind/backups/brainify-purge-$(date +%Y%m%d-%H%M%S)"
...
BACKUP_PATH="$HOME/.voltmind/backups/shared-brain-history-backup-$(date +%Y%m%d-%H%M%S).git"
...
nothing in voltmind mechanically blocks `git filter-repo` — which is exactly why
...
# (under $WORK in ~/.voltmind/backups — never /tmp, which can vanish mid-rewrite).
CLEAN="$WORK/clean"
mkdir -p "$CLEAN"
...
- The sync cursor may reference a rewritten-away SHA; if the next
  `voltmind sync` errors or falls back to a full rescan, that is the cursor
  recovering — run `voltmind doctor` if it doesn't settle
- **Backup retention:** once the rewrite is verified good (team has
  re-cloned, sync settled, no missing content reported), keep the
  mirror-clone backup in `~/.voltmind/backups/` for a retention window
  (~30 days is a sane default), then delete it — it contains the
  pre-sanitization history and should not accumulate indefinitely:
  `rm -rf ~/.voltmind/backups/shared-brain-history-backup-<date>.git`
...
`voltmind config set brain.audience shared` — run against the TEAM brain,
...
fires on it. If team members will run `voltmind init` for the shared brain
BEFORE this stamp exists (e.g. a fresh clone initializing its own engine),
have them prefix that one command with `VOLTMIND_NO_ONBOARD_NUDGE=1` — a
...
cron slot submits a background job (`voltmind jobs submit`), scheduling
...
- **Cross-references:** after sanitizing people pages, check that no other
  ...
  `voltmind check-backlinks check` plus a grep for the removed section titles.
...
- Sensitive fact rows are deleted from the page's Facts fence, re-synced,
  ...
  verified via `voltmind recall --grep`, never merely expired — `voltmind
...
- ❌ Using `voltmind forget` as sanitization — forget expires (struck-through
