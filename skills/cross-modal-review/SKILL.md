...
> **Relationship to `voltmind eval cross-modal`:** This skill is the manual
> mid-flow gate (one model reviews work product before commit, with refusal
> routing). The `voltmind eval cross-modal` command (v0.27.x) is a sibling
...
> for ad-hoc second opinions; use `voltmind eval cross-modal` for the
