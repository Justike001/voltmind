...
description: |
  ...
  behind) from voltmind doctor, autopilot, sync, and cron monitors — runs
...
On voltmind surfaces this covers: `voltmind doctor` staleness checks (e.g. sync
...
- The alert's threshold is compared against the authoritative one
  (`voltmind doctor`'s warn/fail lines) before the system is declared unhealthy.
...
2. **Time the suspect step directly.** Isolate the smallest unit that the alert
   ...
   ```bash
   time voltmind sync --source source-a --no-embed
   ```
   ...
   Cross-check state with `voltmind sources status` (per-source sync lag).
...
4. **Check the alert threshold against the authoritative one.** Before
   ...
   what "bad" means. `voltmind doctor`'s sync-freshness check defaults to
   24h warn / 72h fail (env-overridable via `VOLTMIND_SYNC_FRESHNESS_WARN_HOURS`
   / `VOLTMIND_SYNC_FRESHNESS_FAIL_HOURS`); a cron monitor paging at 12h is
...
- The alert says "no recovery in flight" — verify whether recovery is in fact
  ...
  `voltmind jobs list` for queued work).
...
- **Raising a timeout to fix a stall.** If the step is genuinely hung, a bigger
  ...
  "is wedged" — they have opposite fixes. (voltmind's sync stall watchdog makes
...
- **Triple-wrong diagnosis on a freshness alert.** A cron monitor paged
  ...
  `time voltmind sync --source source-a --no-embed` finished in single-digit
  seconds with "Already up to date" (same for `source-b`), and
  `voltmind sources status` showed every source synced that morning. Every
  theory died at once. Actual cause: the monitor alerted at its
  act-threshold, hours below `voltmind doctor`'s authoritative warn line. The
