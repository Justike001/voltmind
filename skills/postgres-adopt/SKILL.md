...
name: postgres-adopt
description: |
  Detect which voltmind engine is in use (PGLite vs Postgres), prefer Postgres
...
triggers:
  - "which voltmind engine"
  - "pglite or postgres"
  - "voltmind engine status"
  - "upgrade to postgres"
  - "switch voltmind to postgres"
  - "install postgres for voltmind"
...
- Detection is engine-free and read-only: `voltmind engine status --json`
...
- Engine changes NEVER happen by editing config. Moving data uses
  `voltmind migrate --to <supabase|pglite>` — which brings its own guardrails
...
- `config_file_engine` differs from `effective_engine` → an env URL is
  ...
  is `db_url_source` (`env:VOLTMIND_DATABASE_URL` / `env:DATABASE_URL` means
...
- Ask the operator BEFORE adding `--allow-docker` (it creates and owns a
  `voltmind-postgres` container that survives reboots) or `--allow-create-db`
...
```bash
voltmind migrate --to supabase --url <postgres-connection-string>
```
...
- After a clean run: `voltmind doctor` on the new engine; the old
  `brain.pglite/` dir is preserved (doctor's `pglite_leftovers` tracks it).
- `voltmind doctor`'s `pglite_scale` check warns at 1000+ pages — that warning
  is this skill's cue.
...
- NEVER `voltmind config set engine ...` — it is refused by design; an engine
...
Quote `voltmind engine status` output as-is (it is already redacted); name the
winning ladder rung when an install ran; after a migration, include the
target's `voltmind doctor` verdict.
