...
|---|---|
| `voltmind query` (expansion/synthesis) | `voltmind search "<exact token>"` + `voltmind get <slug>` to read the page itself |
| `voltmind search` (hybrid retrieval) | `voltmind graph-query <slug> --type <edge>` or `voltmind backlinks <slug>` |
| graph traversal (`voltmind graph` / `graph-query`) | direct page read (`voltmind get <slug>`) — does the page actually assert this? |
...
```
voltmind graph-query alice-example --type founded
voltmind graph-query fund-a --type invested_in --direction out
```
