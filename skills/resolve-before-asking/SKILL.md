...
```bash
voltmind think "Who is {entity}? What is their relationship to the user? What role do they play? Use all available context — meetings, timeline, imported archives, facts."
```
...
```bash
voltmind search "{entity}" --limit 5
voltmind get {entity-slug}
```
...
voltmind query "emails with {entity}" --limit 10
voltmind query "meetings with {entity}" --limit 5
```
...
```bash
voltmind timeline {entity-slug} --limit 20
voltmind backlinks {entity-slug}
voltmind graph {entity-slug} --depth 2
```
...
3. **Extract:** relationship type (friend / colleague / advisor / family /
   ...
   the user, what era).
4. **Update the page** (`voltmind put`): fill the `relationship` frontmatter
...
page had a placeholder, it is filled (`voltmind put`) as a side effect. No message
