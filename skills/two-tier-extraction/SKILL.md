...
> this skill uses voltmind's tier vocabulary (`utility` / `reasoning` / `deep`).
> Resolve tiers through `voltmind models`; never hardcode a model ID.
>
...
```bash
voltmind models                          # current tier → model table
voltmind config set models.tier.deep opus   # example: pin the escalation tier
```
...
- **Hardcoding model IDs.** Tiers resolve through the model-routing
  convention (`voltmind models`); a hardcoded ID rots and silently breaks.
