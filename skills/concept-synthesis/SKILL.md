...
The skill is markdown agent instructions. The agent uses voltmind's
existing operations + LLM passes:

```bash
# 1. List all concept pages
voltmind query "type:concept" --limit 10000 --json
...
- Run `voltmind query "type:concept"` and spot-check the count reduction.
