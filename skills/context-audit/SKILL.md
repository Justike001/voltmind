...
description: |
  ...
  output. Judging routes through `voltmind eval cross-modal` (single cheap
...
> (`voltmind recall "context audit report"`) so you can compute token DRIFT since
...
- **Rendered-file safety.** Any recommendation touching a bootstrap-rendered
  ...
  (`voltmind bootstrap interview --set KEY "..."` then
  `voltmind bootstrap render --only <FILE> --force`), never as a direct edit.
...
- **Native judging.** The draft report is quality-gated through
  `voltmind eval cross-modal` — no raw model API calls, no hardcoded model IDs.
...
# (`voltmind models` shows all resolved tiers if the config key is unset.)
JUDGE=$(voltmind config get models.tier.utility)
...
- **Recommending a direct edit to a rendered file.** SOUL.md / USER.md /
  ...
  `voltmind bootstrap render`. Target the answer bank or template, then
  re-render.
- **Raw model API calls for judging.** The eval runner owns provider config,
  receipts, and verdict aggregation — route through `voltmind eval cross-modal`.
