...
description: |
  ...
  orchestration. Replaces the older `voltmind-jobs` routing intent. Use when:
  submitting voltmind jobs, shell/background tasks, spawning subagents,
...
triggers:
  - "voltmind jobs submit"
  - "submit a voltmind job"
  - "submit a shell job"
  - "shell job"
...
- Deterministic shell jobs (`voltmind jobs submit shell ...`)
- LLM subagent jobs (`voltmind agent run ...`)
...
| Condition | Action |
|---|---|
| User asks for deterministic command/script run | Shell job (CLI: `voltmind jobs submit shell ...`) |
| User asks to "run in minions" + explicit command/argv | Shell job (CLI, `--params` with `cmd` or `argv`) |
| User asks for research/reasoning/iterative agent | Subagent job (CLI: `voltmind agent run`) |
...
| Parallel work (2+ streams) | `voltmind agent run --fanout-manifest` or parent + child subagents |
...
- **`voltmind_ALLOW_SHELL_JOBS=1` must be set on the worker environment.**
...
- **Security:** flipping `voltmind_ALLOW_SHELL_JOBS=1` authorizes arbitrary
...
- **Execution mode — pick one:**
  - **Postgres + daemon:** `voltmind jobs work` runs a persistent worker that
    claims and executes jobs from the queue.
  - **PGLite + --follow:** `voltmind jobs submit ... --follow` runs inline.
...
- **MCP boundary:** shell-job submission is CLI-only. `submit_job name="shell"`
  ...
  agent observes.
- **Verify setup:** after configuration, run `voltmind jobs stats` (CLI) to
...
Command string form:
```
voltmind jobs submit shell --params '{"cmd":"echo hello","cwd":"/abs/path"}'
```

Argv form (no shell expansion):
```
voltmind jobs submit shell --params '{"argv":["bash","-lc","echo hello"],"cwd":"/abs/path"}'
```

Inline execution on PGLite or any one-shot deployment:
```
voltmind jobs submit shell --params '{"cmd":"echo hello","cwd":"/tmp"}' --follow
```

Queue/lifecycle flags exposed by `voltmind jobs submit --help`: `--queue`,
...
timings) from `get_job`. Use `voltmind jobs stats` (CLI) for worker/queue
health dashboard.
...
**User-facing entrypoint:** `voltmind agent run <prompt>` is the canonical way
...
submission requires `{allowProtectedSubmit: true}`, which `voltmind agent run`
supplies.

## Phase 1: Submit

```
voltmind agent run "Research Acme Corp revenue" --tools "search,query"
```
...
```
voltmind agent run --fanout-manifest companies.json
```
...
Queue/priority/retry tuning is not exposed by `voltmind agent run`; submit the
raw `subagent` handler via `voltmind jobs submit` (requires CLI trust) if you
need those knobs.
...
voltmind jobs stats                  # CLI — queue health dashboard
voltmind agent logs ID --follow      # CLI — streaming transcript + heartbeat
```
...
- Don't spawn > 5 concurrent agents without checking `voltmind jobs stats` first
- For subagent work, don't use `sessions_spawn` with `runtime: "subagent"` when Minions is available (use `voltmind agent run` instead)
- Don't poll `get_job` in a tight loop (use `get_job_progress` for lightweight checks)

## Tools Used

- Submit a background job — `submit_job` (MCP, non-protected names only; shell jobs are CLI-only, subagent jobs via `voltmind agent run`)
...
- Queue stats — `voltmind jobs stats` (CLI; no MCP equivalent)
