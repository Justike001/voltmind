...
> skill's Phase 3 below uses `voltmind eval cross-modal` instead — three
...
receipt under `~/.voltmind/.voltmind/eval-receipts/<slug>-<sha8>.json` (the
...
Models resolve through the voltmind AI gateway. Configure once with:
...
from `~/.voltmind/config.json` plus `process.env`.
...
voltmind skillify check skills/<slug>/scripts/<slug>.mjs --json | \
  jq '.[] | .items[] | select(.name | contains("Cross-modal"))'
ls ~/.voltmind/.voltmind/eval-receipts/              # receipt landed
voltmind check-resolvable --json | jq .ok          # resolver clean
```
...
1. **The skill tree on disk.** `skills/<slug>/SKILL.md`, `scripts/<slug>.mjs`,
   ...
   `voltmind skillify scaffold <name>` and refined by the human/agent into a
   real implementation.
2. **A cross-modal eval receipt** at
   `~/.voltmind/.voltmind/eval-receipts/<slug>-<sha8>.json`. The sha-8 binds the
   receipt to the current `SKILL.md` content. `voltmind skillify check`
   surfaces the status (`found` / `stale` / `missing`) as informational.
3. **An audit verdict** from `voltmind skillify check`: `properly skilled` |
...
JSON output (`voltmind skillify check --json`) includes the same fields plus
