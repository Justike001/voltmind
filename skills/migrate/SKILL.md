...
Universal migration from any wiki, note tool, or brain system into voltmind.

## Contract

- Source data is never modified or deleted; migration is additive only.
- Every migrated page is verified round-trip: written to voltmind, read back, spot-checked.
- Cross-references from the source system (wikilinks, block refs, tags) are converted to voltmind equivalents.
...
|--------|--------|----------|
| Obsidian | Markdown + `[[wikilinks]]` | Direct import, convert wikilinks to voltmind links |
...
| Plain markdown | Any .md directory | Import directory into voltmind directly |
...
2. **Plan the mapping.** How do source fields map to voltmind fields (type, title, tags, compiled_truth, timeline)?
3. **Test with a sample.** Import 5-10 files, verify by reading them back from voltmind and exporting.
4. **Bulk import.** Import the full directory into voltmind.
5. **Verify.** Check voltmind health and statistics, spot-check pages.
6. **Build links.** Extract cross-references from content and create typed links in voltmind.

## Obsidian Migration

1. Import the vault directory into voltmind (Obsidian vaults are markdown directories)
...
Obsidian-specific:
- Tags (`#tag`) become voltmind tags
- Frontmatter properties map to voltmind frontmatter
...
5. Import the cleaned directory into voltmind
...
4. Store each page in voltmind

## Verification

After any migration:
1. Check voltmind statistics to verify page count matches source
2. Check voltmind health for orphans and missing embeddings
3. Export pages from voltmind for round-trip verification
4. Spot-check 5-10 pages by reading them from voltmind
5. Test search: search voltmind for "someone you know is in the data"
...
- **Ignoring cross-references.** Wikilinks, block refs, and tags from the source system must be converted to voltmind equivalents. Dropping them loses the knowledge graph.
...
```
MIGRATION REPORT -- [source] -> voltmind
=======================================
...
- Store/update pages in voltmind (put_page)
- Read pages from voltmind (get_page)
- Link entities in voltmind (add_link)
- Tag pages in voltmind (add_tag)
- Get voltmind statistics (get_stats)
- Check voltmind health (get_health)
- Search voltmind (query)
