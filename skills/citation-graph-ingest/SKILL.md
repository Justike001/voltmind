...
description: |
  ...
  *overrules* B, *distinguishes* C, or *relies_on* D. This skill extracts every
  ...
  writes first-class typed edges via `voltmind link`, so `voltmind graph-query
...
> — resolve slugs and read documents through voltmind tools before anything else;
...
native `links` table via `voltmind link` (stamped `link_source=citation-graph`);
that is why the frontmatter carries `writes_pages: false` and no `writes_to:`
list.

## What it is (and is NOT)

- **NOT new storage.** voltmind already has a typed `links` table, a native
  `voltmind link` command (alias: `link-add`), and a `graph-query --type` walker.
...
- **The citation-graph signature is the `link_type`** — `overrules /
  ...
  outside voltmind's standard `attended` / `works_at` / `mentions` set).
...
- **Stamp provenance:** pass `--link-source citation-graph` on every edge. The
  ...
  dedicated tag makes the graph auditable (`voltmind link-sources`) and
  bulk-removable (`voltmind unlink <from> <to> --link-source citation-graph`)
...
- **Typed edges, created natively.** Every inter-document reference that
  survives classification is written with `voltmind link <from> <to> --link-type
...
- **Queryable via graph-query.** The written edges are traversable with
  `voltmind graph-query <slug> --type <type> --direction in|out|both` — this is
  the retrieval surface the skill delivers.
- **Plainly stated limitation:** natural-language relational retrieval (the
  relational-recall arm inside `voltmind query`, e.g. "who invested in X")
  ...
  **explicit graph queries + link hygiene** — do not promise users that
  `voltmind query "is doc A still authoritative?"` will walk these edges.
...
The corpus must already be ingested as a voltmind source so slugs exist
(`voltmind sources add` + `voltmind sync`, or `voltmind import`). Confirm scope:
`--source <name>`, `VOLTMIND_SOURCE`, or a `.voltmind-source` dotfile. Every
...
as context. Use `voltmind search` / `get_page` to enumerate corpus pages and
...
`voltmind get <slug>` when the sentence is ambiguous) and pick the single best
...
```bash
voltmind link doc-b-example doc-a-example \
  --link-type extends \
...
```bash
voltmind graph-query doc-a-example --direction in --source <corpus-source>
voltmind graph-query doc-a-example --type overrules --direction in --source <corpus-source>
```
...
```bash
voltmind link-sources          # citation-graph should appear with the expected count
voltmind check-backlinks check # confirm no orphaned references
```
...
paragraphs and cannot answer; `voltmind graph-query doc-a-foundation --type
...
## Verified walk
<paste the `voltmind graph-query` output from the hub document>

## Hygiene
- `voltmind link-sources`: citation-graph = N edges
...
- **Promising relational-recall answers.** Do not tell users that
  natural-language `voltmind query` will traverse citation edges — it walks a
...
- Native `enrich` entity extraction — creates person/company edges
  (`works_at`, `invested_in`); `voltmind edges-backfill` creates code-symbol
