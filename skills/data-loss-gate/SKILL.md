...
> `voltmind graph <slug>`, `git log` on the underlying files. The confirmation
...
follows it. It is NOT an operation-boundary enforcement: nothing in the voltmind
...
gates — e.g. `voltmind sources remove` requires `--confirm-destructive` — but
...
Brain / database level (voltmind-specific):

- **Bulk forget** — scripting or looping `voltmind forget <fact-id>` over many
  facts. One forget is a considered, idempotent act; a forget sweep is data loss.
- **Page-delete sweeps** — `voltmind delete <slug>` in a loop, or any script that
  sweeps `delete_page` across a set of slugs. Deletes are soft (recoverable via
  `voltmind restore <slug>`) until purged — say so on the card, then gate anyway:
  a sweep that's wrong in bulk is expensive to un-wrong in bulk.
- **`voltmind purge-deleted`** — permanently removes soft-deleted pages. This is
  the point of no return for the soft-delete safety net.
- **Source removal** — `voltmind sources remove <id>` deletes the source AND
  every page in it. The `--confirm-destructive` flag does not substitute for
  the card.
- **Mount removal** — `voltmind mounts remove <id>` only removes the local
...
- [ ] Soft-deleted in the brain (restorable via `voltmind restore` until purged)
