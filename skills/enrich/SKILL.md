...
- `voltmind search "name"` and `voltmind query "what do we know about name"`
...
Store raw API responses via `put_raw_data` in voltmind:
```json
{
  "source": "crustdata",
...
- Store raw API data in voltmind (put_raw_data)
- Retrieve raw data from voltmind (get_raw_data)
