...
(SkillOpt), and a **replay + judge harness** (`voltmind eval cross-modal`). The
generator and the judge ship with voltmind. The persistently missing leg is the
...
1. **Brain conversation archive** — pages under `conversations/`, populated by
   ...
   ```bash
   voltmind search "<skill-name>"
   voltmind query "when did I use <skill-name> and what did I ask for"
   ```

2. **Per-harness session transcripts, as available** — use what the harness
   exposes; do not assume a layout. `voltmind transcripts recent --full` reads
...
`voltmind skillopt <name> --bootstrap-from-skill` (spec-derived, and honest
about it) — see Dedup.
...
2. Convert approved cases into `skills/<name>/skillopt-benchmark.jsonl` lines
   and run `voltmind skillopt <name>` — this is the SkillOpt surface extension:
   a history-grounded benchmark replacing the spec-derived bootstrap.
3. Judge outputs through the native gate:

   ```bash
   voltmind eval cross-modal --task "<what the output was meant to achieve>" --output <path>
   ```
...
`voltmind eval cross-modal` already exits 2 (INCONCLUSIVE) when fewer than 2/3
...
| PROMPT-FIXABLE | The correction targets tone, format, or an omission the SKILL.md could specify | An eval case + a `voltmind skillopt` run |
...
- **Substrates:** `conversations/` archive pages (brain-first), then
  per-harness session transcripts as available (`voltmind transcripts recent`
...
- **Cost posture:** cheap-model default for mining and synthesis. The full
  ...
  opt-in, and judging goes through native `voltmind eval cross-modal` — no
...
- ❌ Building a bespoke judging harness when `voltmind eval cross-modal` is the
  native gate.
...
- **BrainBench (`voltmind eval` suites)** — evals the ENGINE (retrieval,
...
- **`skills/cross-modal-review/SKILL.md` / `voltmind eval cross-modal`** — RUN
