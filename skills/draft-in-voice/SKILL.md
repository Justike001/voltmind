...
1. **The validated voice profile** — `voltmind get people/<slug>-voice`. The
...
2. **First-party context** — the subject's main page (`voltmind get
   people/<slug>`) plus timeline and backlinks (`voltmind timeline <slug>`,
   `voltmind backlinks people/<slug>`): how they actually frame their work,
...
3. **(optional) Topic-specific pages** — if the draft is about a specific
   idea or company, pull the relevant page (`voltmind search "<topic>"`, then
   `voltmind get <slug>`) so every claim is accurate, not invented.
...
- Check what the brain already holds before asking for uploads:
  `voltmind search "<person name>"`, `voltmind backlinks people/<slug>`, and any
...
`_brain-filing-rules.md`), via `voltmind put people/<slug>-voice` with the page
...
profile_version: 1       # bump on every rebuild; prior versions via `voltmind history`
built_at: 2026-08-11
validated_at: null
validated_by: null
consent:
...
Back-link the profile from the subject's main page (`voltmind link
...
- **Versioning:** every rebuild bumps `profile_version` and re-runs the blind
  check. `voltmind history people/<slug>-voice` is the audit trail.
