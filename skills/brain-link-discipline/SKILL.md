...
| Inside a brain page body | RELATIVE markdown link: `[Alice Example](../people/alice-example.md)` | voltmind's link extraction builds the links/backlinks graph — which powers relational retrieval — from filesystem-relative links. An absolute URL between two brain pages is invisible to that graph |
...
`voltmind check-backlinks check` audits the graph and `voltmind sync --no-pull`
...
2. **Repo-relative path + scope note.** No hosted remote (the default PGLite
   ...
   in the brain repo.
3. **`voltmind publish` output as an attachable HTML ARTIFACT.** `voltmind
...
to the `voltmind publish` artifact (step 3 of the fallback chain).
...
> one with `voltmind publish` and attach the file.
...
- ❌ Presenting `voltmind publish` output as a URL. It emits a local HTML file
