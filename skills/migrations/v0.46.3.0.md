...
feature_pitch:
  ...
  one_liner: "One command migrates the brain (embeddings AND reranker): voltmind migrate embeddings --to voyage:voyage-4 --dim 1024"
...
- "doctor says provider_sunset" / "voltmind is warning about a provider"
- after any `voltmind upgrade` that printed the ACTION REQUIRED sunset block

Also proactively surface it when `~/.voltmind/migrations/pending-host-work.jsonl`
contains a `"migration": "0.46.3"` entry.

## Step 0 — env preflight (do this FIRST, it prevents the worst failure mode)

```bash
env | grep VOLTMIND_EMBEDDING
```

`VOLTMIND_EMBEDDING_MODEL` / `VOLTMIND_EMBEDDING_DIMENSIONS` **override the file
plane at runtime** for every voltmind process. Three cases:
...
- **Set and equal to the target** (e.g. `voyage:voyage-4` / `1024`): the
  ...
  env in sync everywhere voltmind runs (cron, workers, other shells) — or
  `unset` it so the file plane is the single source of truth. Env-canonical
  deployments (containers with no `~/.voltmind/config.json`) are supported: the
...
- **Set and different from the target**: the live run REFUSES (this is the
  ...
  ```bash
  unset VOLTMIND_EMBEDDING_MODEL VOLTMIND_EMBEDDING_DIMENSIONS
  ```
...
```bash
voltmind jobs list --status running 2>/dev/null; voltmind jobs list --status waiting 2>/dev/null
```
...
```bash
voltmind doctor --json | jq '.checks[] | select(.name=="provider_sunset")'
```

`warn`/`fail` mentioning zeroentropyai = exposed. `ok` = likely done — verify
with `voltmind migrate embeddings --status` (Step 5) before clearing the
...
`~/.voltmind/config.json`). One key covers embedding + reranking + the
multimodal model. To set the key: `export VOYAGE_API_KEY=...` or edit
`~/.voltmind/config.json` directly — do NOT use `voltmind config set
voyage_api_key` (that writes the DB plane, which the embedding pipeline never
reads).

```bash
voltmind migrate embeddings --to voyage:voyage-4 --dim 1024 --dry-run   # cost preview — show the user
voltmind migrate embeddings --to voyage:voyage-4 --dim 1024 --yes
```
...
```bash
voltmind migrate embeddings --to openai:text-embedding-3-small --dim 1280 --dry-run
voltmind migrate embeddings --to openai:text-embedding-3-small --dim 1280 --yes
...
```bash
voltmind config set search.reranker.model voyage:rerank-2.5   # needs VOYAGE_API_KEY
voltmind config set search.reranker.enabled false             # or turn it off
```
...
`voltmind config set embedding_model`), while reranker config lives on the DB
plane (`voltmind config set search.reranker.*` is correct there). The migration
...
search still works, ordering quality drops. Past the shutdown date voltmind
...
```bash
voltmind migrate embeddings --status
```
...
```bash
voltmind doctor --json | jq '.checks[] | select(.name=="provider_sunset") | .status'   # → "ok"
voltmind search "anything you know is in the brain"                                     # sanity check
```

Facts note: fact vectors regenerate on their next write / `voltmind extract`
pass — `--status` shows the pending count; it is not a failure.

Then remove/mark the `0.46.3` entry in
`~/.voltmind/migrations/pending-host-work.jsonl` as done — edit the file and
...
- **Killed / crashed mid-run** → re-run the SAME command. The NULL-embedding
  ...
  re-billed. `voltmind migrate embeddings --status` shows the in-flight marker
  and prints the exact resume command.
- **"Migration paused ... lock"** → another embed backfill holds a per-source
  lock. Check `voltmind jobs list`; a hard-killed run's lock expires within 60
  minutes — re-run then.
...
- **Deferred the re-embed with `--no-embed`** → finish with:
  `voltmind embed --stale --catch-up --include-null-signature`
  (`--background` carries all of these flags into the job).
- **Only want status, never mutation** → `voltmind migrate embeddings --status`
...
```bash
voltmind config set provider_base_urls.zeroentropyai http://your-host:PORT/v1
```
...
Note: `voltmind migrate embeddings` refuses `--to zeroentropyai:*` by default
...
- `voltmind migrate embeddings --status` shows convergence on a non-ZE target
