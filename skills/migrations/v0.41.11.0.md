...
2. New CLI command `voltmind extract-conversation-facts` for bulk fact extraction over long-form conversation pages.
...
2. **Run a dry-run** to preview segmentation:
   ```bash
   voltmind extract-conversation-facts --dry-run --slug <slug-of-one-conversation-page>
   ```
...
3. **Run with explicit cost cap.** Default cap is $5. Recommend matching the audit estimate (rounded up):
   ```bash
   voltmind extract-conversation-facts --source-id <id> --background --max-cost-usd <cap>
