...
```bash
voltmind search "<relevant terms>" --limit 10
```

For concept-shaped or synonym-phrased claims, escalate to `voltmind query
...
```bash
BRAIN_DIR=$(voltmind config get sync.repo_path)
...
**Important:** on voltmind installs these files are RENDERED from the bootstrap
...
```bash
voltmind recall <entity-slug>            # facts about the subject, newest first
voltmind recall --grep "<claim terms>"   # substring filter when the entity is unclear
```
...
| **FACTS_TABLE_ERROR** | Wrong fact in the voltmind facts table | `recall` → `forget <fact-id>` → `remember` the correction |
...
- **BRAIN_ERROR:** Edit the page file in the brain repo. Include
  ...
  `voltmind sync` so the DB reflects the fix. (Editing the DB row without the
...
- **SOUL_USER_ERROR:** NEVER edit SOUL.md / USER.md directly — they are
  ...
  ```bash
  voltmind bootstrap interview --set KEY "corrected value"   # verbatim, user's words
  voltmind bootstrap interview --show                        # read back
  voltmind bootstrap interview --status                      # get the confirm hash
  voltmind bootstrap interview --confirm <hash>
  voltmind bootstrap render --only SOUL.md --force           # repeat per affected file
  ```
...
- **FACTS_TABLE_ERROR:** Expire the wrong row and write the correction with
  provenance:
  ```bash
  voltmind recall <entity-slug>                                  # find the fact id
  voltmind forget <fact-id>                                      # expire the wrong fact
  voltmind remember "<correct fact>" \
...
- **HALLUCINATION:** There is no source to fix. Identify the partial signal
  ...
  ```bash
  voltmind remember "WRONG: <what was said>. RIGHT: <what is true>. Guard: <instruction to prevent recurrence>" \
...
voltmind search "<wrong claim terms>" --limit 20
```
...
```bash
voltmind eval suspected-contradictions   # run the probe
voltmind find-contradictions             # read the latest run's findings
```
...
- Propagation is checked (whole-brain grep + `voltmind search`)
...
- The contradictions probe (`voltmind eval suspected-contradictions` /
  `voltmind find-contradictions`) — PROACTIVE intra-brain conflict detection.
