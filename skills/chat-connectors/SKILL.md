...
> check what the brain already has (`voltmind search`/`query`/`get`); sync is
...
the same ingest pipeline (`voltmind transcripts ingest` under the hood), so
...
(`~/.voltmind/connectors/<provider>.json`, mode 0600) and is sent only to the
provider's own host.

```bash
# ChatGPT: copy the Cookie request header from DevTools (Network tab), then:
voltmind connectors auth chatgpt --cookie -      # paste, Ctrl-D  (keeps it out of argv)

# Claude: copy the sessionKey cookie value from DevTools (Application → Cookies):
voltmind connectors auth claude --cookie -       # paste `sessionKey=<value>`, Ctrl-D
```

Every auth run ends with a probe + a one-line verdict. Nothing is saved on a
failed probe unless you pass `--force`. `voltmind connectors auth chatgpt
...
export instead — Settings → Export data → `voltmind transcripts ingest
conversations.json` (the conversation-archive lane).

## First Sync

```bash
voltmind connectors sync chatgpt --dry-run       # preview: how many conversations
voltmind connectors sync chatgpt --limit 5       # import a small sample first
voltmind conversation-parser scan conversations/chatgpt/<slug>   # validate one page
voltmind connectors sync chatgpt --full          # import everything
```
...
```bash
voltmind config set connectors.chatgpt.auto_sync true    # opt in (per provider)
voltmind autopilot --install                             # harness-agnostic scheduler
```

`voltmind autopilot --install` picks launchd / systemd / crontab / a container
start-script for you and runs the tick that dispatches the sync (credential-gated
+ auto_sync-gated; a dead cookie stops it and surfaces in `voltmind doctor`).

**Daemonless hosts** (no autopilot): a host cron line works too —
`0 6 * * * voltmind connectors sync --all` (daily). Tune the floor with
`voltmind config set connectors.sync_floor_min <minutes>` (default 1440).

## Credential Care

- Credentials live at `~/.voltmind/connectors/<provider>.json` (0600), never in the
  DB, `sources.config`, or any MCP payload. `VOLTMIND_CONNECTOR_<PROVIDER>_COOKIE`
  overrides the file (incident escape hatch).
- `voltmind connectors status` shows provenance + expiry (never the value).
- `voltmind connectors logout <provider>` deletes a credential.
- Cookies expire (days to weeks). When one dies, sync stamps `auth_error_at`,
  the scheduler stops burning slots, and `voltmind doctor` says "re-auth needed" —
  re-run `voltmind connectors auth`.
...
- Credentials are stored file-plane at `~/.voltmind/connectors/<provider>.json`
...
- Health (re-auth-needed / stalled / drift) surfaces in `voltmind doctor`.
