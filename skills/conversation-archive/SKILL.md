...
1. **IMPORT** — raw export or session log → dated markdown pages under
   ...
   conversation, then `voltmind import`/`voltmind sync`) → parser validation →
...
**A native importer now exists: `voltmind transcripts ingest`.** It parses
...
```
voltmind transcripts ingest ~/Downloads/conversations.json   # unzip first
voltmind transcripts ingest                                  # discover harness logs
voltmind transcripts ingest --max-bytes 4gb <store>          # oversized store (omit = per-format caps)
voltmind transcripts status                                  # found vs imported gaps
```
...
patterns) plus your `~/.voltmind/harvest-private-patterns.txt` regexes and
...
so `YYYY-MM-DD-new-chat` collides across threads. `voltmind put` has no
...
slug, and check-before-write (`voltmind get <slug>`) — a hit that is NOT the same
...
The model is voltmind's own `~/.voltmind` deny-list / `runPrivacyLint` pattern
...
- `type: conversation` is REQUIRED — it is what makes the page eligible for
  `voltmind extract-conversation-facts`.
- Message lines use `**Speaker:** text` (parses via the built-in
  ...
  inline dates). Run `voltmind conversation-parser list-builtins` to see every
...
- Pages written inside the brain repo: `voltmind sync --no-pull`
- Standalone conversion directory: `voltmind import <dir> --source-id <id>`
...
```bash
voltmind conversation-parser scan conversations/chatgpt/2025-03-15-agent-memory
```
...
voltmind extract-conversation-facts --types conversation --dry-run --limit 5

# Real run, cost-capped; use --background for large archives
voltmind extract-conversation-facts --types conversation --max-cost-usd 5
```

This is the shipped batch extractor (`voltmind extract-conversation-facts
...
Related native surface: `voltmind transcripts recent --days 7` reads recent raw
...
- **Find a conversation:**
  `voltmind search "<what you remember>" --limit 20` — then filter results to
  `conversations/` slugs (prefix per provider: `conversations/chatgpt/`, …).
- **Pull a thread:** `voltmind get conversations/chatgpt/2025-03-15-agent-memory`
- **"When did I first discuss X":**
  1. `voltmind query "X" --limit 50` and sort `conversations/` hits by the
     slug's date prefix.
  2. Probe earlier: `voltmind query "X" --until <earliest-date-found>` and
...
- **Context around a date:** `voltmind day 2025-03-15` shows what else happened
  that day; `voltmind recall --query "X"` checks the extracted-facts arm.
...
- ❌ Overwriting a colliding slug (same-day "New chat") — suffix a short thread
  hash; `voltmind put` has no CAS, so a blind write silently loses the first thread
- ❌ Inventing a message line format the parser can't read — validate with
  `voltmind conversation-parser scan` before bulk-converting
...
- ❌ Declaring "you never discussed X" after one failed search — try
  synonyms, check `voltmind recall`, and only then answer in the negative
...
- **[capture](../capture/SKILL.md)** — the single-item front door
  (`voltmind capture` → `inbox/`). One pasted snippet routes there; a corpus of
...
- Every import run validates a sample via `voltmind conversation-parser scan`
  before bulk conversion, and reports parser results in the import receipt.
- Fact extraction goes through the native `voltmind extract-conversation-facts`
