...
2. **Execute searches:**
   ...
   - List pages in voltmind by type or check backlinks for structural queries
3. **Read top results.** Read the top 3-5 pages from voltmind to get full context.
...
- `voltmind graph-query <slug> --type <link_type> --depth N --direction in|out|both`
...
Examples:
- "Who works at Acme?" → `voltmind graph-query companies/acme --type works_at --direction in`
- "Who attended Demo Day W26?" → `voltmind graph-query meetings/demo-day-w26 --type attended --direction out`
- "What companies has Emily advised?" → `voltmind graph-query people/emily --type advises --direction out`
- "Who has Alice met (via meetings)?" → `voltmind graph-query people/alice --type attended --depth 2`

Combine with `voltmind query` for queries that need BOTH semantic similarity AND
...
- Run `voltmind doctor --json` to check index health
- Check embedding coverage -- partial embeddings degrade hybrid search
- Compare keyword search (`voltmind search`) vs hybrid search (`voltmind query`)
...
- Keyword search voltmind (search)
- Hybrid search voltmind (query)
...
- Traverse the link graph in voltmind (traverse_graph)
