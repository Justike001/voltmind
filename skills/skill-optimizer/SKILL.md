...
- **Bundled skills require explicit opt-in AND an independent held-out set.**
  Skills shipping with voltmind cannot be auto-mutated. To rewrite one in place
...
```
voltmind skillopt <skill-name> [flags]
  │
  ├── Pre-flight gates
...
4. **Run the optimizer with `--split 1:1:1`:**
   ```
   voltmind skillopt X --bootstrap-reviewed --split 1:1:1
   ```
...
| Mid-run crash | `voltmind skillopt foo --resume <run-id>` |
...
- `~/.voltmind/audit/skillopt-YYYY-Www.jsonl` — ISO-week-rotated audit trail
...
- **Don't optimize bundled skills without `--allow-mutate-bundled` AND
  `--held-out`.** They ship with voltmind and are load-bearing for downstream
