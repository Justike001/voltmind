...
back-links get created (`voltmind link` / `add_link`). An unlinked mention is
...
- **Manifest-driven** — progress state lives in the manifest, not in the
  process's memory.
- **Minion-friendly** — runnable as `voltmind jobs submit shell` payloads or
  `voltmind agent run` subagents (Phase 9).
...
`voltmind check-resolvable`, cross-modal eval, and brain filing registration.
...
voltmind jobs submit shell --params '{"cmd": "<your pipeline command> --offset 0 --limit 100"}'

# LLM-heavy pipeline as a subagent (steerable, transcripted):
voltmind agent run "Read skills/<pipeline-name>/SKILL.md and process the next 50 pending manifest items"
```

Shell jobs require `VOLTMIND_ALLOW_SHELL_JOBS=1` on the WORKER environment — see
...
disk. After the bulk run: `voltmind sync` to index everything, then
`voltmind check-backlinks check` to catch propagation gaps.
...
□ voltmind sync run after bulk write; check-backlinks clean
...
- **Native `voltmind sync`** — checkpointed file sync for brain repo sources.
