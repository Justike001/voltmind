...
```bash
voltmind jobs submit shell --params '{"cmd": "cd <brain-repo> && git add projects/<pipeline-name> <output-dirs> && git commit -m \"<pipeline-name> ingest checkpoint\" && git push"}'
```
...
env prefix (`VOLTMIND_ALLOW_SHELL_JOBS=1 voltmind jobs submit ...`) is a no-op in
...
- **Native `voltmind sync` checkpoints** cover resumable file sync for brain
  ...
  knows nothing about.
- **Minion job progress** (`voltmind jobs`) is per-job and DB-backed; the
