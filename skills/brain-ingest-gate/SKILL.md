...
> the lookup chain (`voltmind entity` → `search` → `query` → `get`) is the same
...
`voltmind put`, subagent output), two checks run in order:
...
follows it. Nothing in the voltmind runtime mechanically blocks an unenriched or
duplicate write if the skill never loads.

## Why voltmind needs this gate

The native pipeline does NOT do semantic dedup for you:

- **`voltmind import` / `voltmind sync` skip only matching frontmatter IDs.**
...
- **`voltmind capture`'s dedup is a 24-hour exact content-hash** — it catches
...
2. **Bulk imports** — batch moves of any kind into brain directories, BEFORE
   `voltmind sync` or `voltmind import` indexes them. For batches, also read
...
3. **Manual writes** — `voltmind put` or `voltmind capture` of rich content, or
...
1. **Resolve registry-first, not by the generic phrase.** voltmind's native
   registry is the entity surface:

   ```bash
   voltmind entity "<name>"        # zero-LLM card: page, aka list, near-miss suggestions
   ```

   A card hit means the page exists — STOP, link, don't clone. On a miss (or
   for concept-shaped nouns), fall through to `voltmind query "<name>" --limit 3`.
...
3. **A vector score is a floor for prose, NEVER a gate for named things.**
   ...
   (`voltmind get <slug>`) before concluding it doesn't exist. A named page can
...
4. **When a NEW named thing appears, bake its aliases in the same write.**
   ...
   resolves through `voltmind entity`. One frontmatter list covers all future
...
2. **Search for it:**
   ...
   ```bash
   voltmind search "<core claim>" --limit 5
   ```

3. **OPEN AND READ the top hit** (`voltmind get <slug>`). Never band on the
   score alone. Donor systems publish cosine cutoffs for this step — do NOT
   port them: `voltmind search` returns fused hybrid rank scores, not cosine
...
4. **Assign a band:**
   ...
   | Band | Meaning | Action |
   |---|---|---|
   | **clear-dup** | The top hit already states the same insight about the same subject | STOP. Link to the existing page (`voltmind link` / `voltmind timeline-add`) instead of writing. |
...
New content to write
  ...
  ├─ voltmind search "<core claim>" --limit 5
  └─ OPEN AND READ the top hit (voltmind get <slug>)
...
```bash
voltmind check-backlinks check            # mentioned entities link back (fix with: check-backlinks fix)
voltmind backlinks <new-slug>             # each new page has inbound links
voltmind search "<core claim>" --limit 3  # the insight has exactly ONE home
```
...
- Dedup bands are assigned by READING the top hit, never by score alone; no
  numeric similarity thresholds are used against voltmind's fused scores.
...
- Batches end with a `voltmind check-backlinks check` verification pass.
...
- ❌ Bulk `mv` of a folder into the brain repo, then `voltmind sync` — sync
...
- ❌ Porting numeric dedup thresholds from other systems onto voltmind's fused
  scores.
...
- ❌ Skipping the gate because the write is "just one page" via `voltmind put` —
