...
function OAuthPage({ initialSource }: { initialSource: string | null }) {
  const [clients, setClients] = useState<OAuthClient[]>([]); const [sources, setSources] = useState<SourceSummary[]>([]); const [sourceId, setSourceId] = useState(initialSource ?? ''); const [status, setStatus] = useState<Exclude<OAuthClientStatus, 'all'>>('active'); const [creating, setCreating] = useState(false); const [secret, setSecret] = useState<SecretResult | null>(null); const [error, setError] = useState('');
  const load = useCallback(async () => { setError(''); try { const [c, s] = await Promise.all([api.oauthClients({ sourceId: sourceId || undefined, status }), api.sources()]); setClients(c.data.filter(client => status === 'active' ? !client.deleted_at : !!client.deleted_at)); setSources(s.data); } catch (cause) { setError(errorText(cause)); } }, [sourceId, status]);
  useEffect(() => { void load(); }, [load]);
  return <section className="page"><PageHeader eyebrow="Credential lifecycle" title="OAuth Clients" description="Source-bound clients. Secrets are displayed once and never persisted." action={<button className="primary-button" onClick={() => setCreating(true)} disabled={!sources.length}>New client</button>} />
    ...
    {error && <ErrorBanner error={error} onRetry={load} />}<OAuthTable clients={clients} onChanged={load} onSecret={setSecret} />
