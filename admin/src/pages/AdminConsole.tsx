...
function OverviewPage({ onNavigate }: { onNavigate: (view: View) => void }) {
  ...
  const [gogs, setGogs] = useState<GogsStatus | null>(null);
  const [error, setError] = useState('');
  const load = useCallback(async () => {
    setError('');
    try {
      const [o, a, s] = await Promise.all([api.overview(), api.autopilot(), api.sources(true)]);
      setOverview(o.data); setAutopilot(a.data); setSources(s.data);
      const candidate = s.data.find(item => !item.archived);
      if (candidate) api.gogs(candidate.id).then(r => setGogs(r.data)).catch(() => setGogs(null));
  ...
  return <section className="page"><PageHeader eyebrow="Host overview" title="Control plane at a glance" description="Sources, credentials, queues, and runtime health from the Ubuntu Host." action={<button className="secondary-button" onClick={load}>Refresh</button>} />
    ...
    <div className="overview-panels">
      ...
      <article className="panel"><div className="panel-title"><h2>Repository health</h2>{gogs && <StatusBadge value={gogs.api_state} />}</div>{gogs ? <dl className="fact-list"><div><dt>Repository</dt><dd>{gogs.repository_owner}/{gogs.repository_name}</dd></div><div><dt>Clone</dt><dd>{gogs.clone_state}</dd></div><div><dt>Last sync</dt><dd>{formatTime(gogs.last_sync_at)}</dd></div></dl> : <Empty>{sources.length ? 'Health probe unavailable; source data remains usable.' : 'No active source to probe.'}</Empty>}</article>
    </div>
  </section>;
}
...
function SourceWorkspace({ sourceId, onBack, onChanged }: { sourceId: string; onBack: () => void; onChanged: () => void }) {
  const [detail, setDetail] = useState<SourceDetail | null>(null); const [gogs, setGogs] = useState<GogsStatus | null>(null); const [gogsError, setGogsError] = useState(''); const [error, setError] = useState(''); const [busy, setBusy] = useState(false); const [secret, setSecret] = useState<SecretResult | null>(null);
  ...
  return <section className="page"><button className="back-button" onClick={onBack}>← All sources</button><PageHeader eyebrow={`SourceID · ${sourceId}`} title={detail?.name ?? sourceId} description="Repository health, OAuth bindings, and lifecycle controls in one workspace." action={<div className="button-row">{!archived && <button className="primary-button" onClick={createClient} disabled={busy}>New client</button>}<button className={archived ? 'secondary-button' : 'danger-button'} onClick={mutateArchive} disabled={busy || sourceId === 'default'}>{archived ? 'Restore source' : 'Archive source'}</button></div>} />
    {error && <ErrorBanner error={error} onRetry={load} />}{!detail ? <Loading /> : <><div className="summary-strip"><div><span>Status</span><StatusBadge value={archived ? 'archived' : 'active'} /></div><div><span>Federation</span><strong>{detail.federated ? 'Enabled' : 'Off'}</strong></div><div><span>Clone</span><strong>{String(detail.clone_state ?? '—')}</strong></div><div><span>Last sync</span><strong>{formatTime(detail.last_sync_at as string | null)}</strong></div></div>
      ...
      <h2 className="section-title">Bound OAuth clients</h2><OAuthTable clients={detail.oauth_clients ?? []} onChanged={load} onSecret={setSecret} /></>}
    {secret && <SecretModal result={secret} onClose={() => setSecret(null)} />}
  </section>;
}

function OAuthPage({ initialSource }: { initialSource: string | null }) {
  const [clients, setClients] = useState<OAuthClient[]>([]); const [sources, setSources] = useState<SourceSummary[]>([]); const [sourceId, setSourceId] = useState(initialSource ?? ''); const [secret, setSecret] = useState<SecretResult | null>(null); const [error, setError] = useState('');
  const load = useCallback(async () => { setError(''); try { const [c, s] = await Promise.all([api.oauthClients(sourceId || undefined), api.sources()]); setClients(c.data); setSources(s.data); } catch (cause) { setError(errorText(cause)); } }, [sourceId]);
  useEffect(() => { void load(); }, [load]);
  const create = async () => { const sid = sourceId || sources[0]?.id; if (!sid) return; const name = prompt('Client name'); if (!name) return; try { setSecret((await api.createOAuthClient(sid, name, 'read write')).data); await load(); } catch (cause) { setError(errorText(cause)); } };
  return <section className="page"><PageHeader eyebrow="Credential lifecycle" title="OAuth Clients" description="Source-bound clients. Secrets are displayed once and never persisted." action={<button className="primary-button" onClick={create} disabled={!sourceId && !sources.length}>New client</button>} /><div className="toolbar"><label>Source <select value={sourceId} onChange={e => setSourceId(e.target.value)}><option value="">All sources</option>{sources.map(s => <option key={s.id} value={s.id}>{s.name} · {s.id}</option>)}</select></label><button className="secondary-button" onClick={load}>Refresh</button></div>{error && <ErrorBanner error={error} onRetry={load} />}<OAuthTable clients={clients} onChanged={load} onSecret={setSecret} />{secret && <SecretModal result={secret} onClose={() => setSecret(null)} />}</section>;
}

function OAuthTable({ clients, onChanged, onSecret }: { clients: OAuthClient[]; onChanged: () => void; onSecret: (result: SecretResult) => void }) {
  const [error, setError] = useState(''); const act = async (client: OAuthClient, action: 'rotate' | 'revoke') => { if (!confirm(action === 'rotate' ? 'Rotate creates a new Client ID and revokes the old client and tokens. Continue?' : 'Revoke this client and all of its tokens?')) return; try { if (action === 'rotate') onSecret((await api.rotateOAuthClient(client.client_id)).data); else await api.revokeOAuthClient(client.client_id); onChanged(); } catch (cause) { setError(errorText(cause)); } };
  if (!clients.length) return <Empty>No OAuth clients found.</Empty>;
  return <>{error && <ErrorBanner error={error} />}<div className="table-card"><table><thead><tr><th>Client</th><th>Source</th><th>Scope</th><th>Status</th><th>Created</th><th>Actions</th></tr></thead><tbody>{clients.map(client => <tr key={client.client_id}><td><strong>{client.client_name}</strong><code title={client.client_id}>{shortId(client.client_id, 24)}</code></td><td><code>{client.source_id}</code></td><td>{client.scope}</td><td><StatusBadge value={client.deleted_at ? 'revoked' : 'active'} /></td><td>{formatTime(client.created_at)}</td><td>{!client.deleted_at && <div className="button-row"><button className="table-button" onClick={() => act(client, 'rotate')}>Rotate</button><button className="table-button danger" onClick={() => act(client, 'revoke')}>Revoke</button></div>}</td></tr>)}</tbody></table></div></>;
}
...
function JobsPage({ initialSource }: { initialSource: string | null }) {
  ...
  useEffect(() => { Promise.all([api.sources(), api.cycleProfiles()]).then(([s, p]) => { setSources(s.data); setProfiles(p.data); if (!sourceId && s.data[0]) setSourceId(s.data[0].id); }).catch(cause => setError(errorText(cause))); }, []);
  const load = useCallback(async () => { if (!sourceId) return; setError(''); try { const response = await api.sourceJobs(sourceId, { status: status || undefined, limit: 50, cursor: cursor ?? undefined }); setJobs(response.data); setNextCursor(response.meta.next_cursor ?? null); } catch (cause) { setError(errorText(cause)); } }, [sourceId, status, cursor]);
...
function AuditPage() {
  const [entries, setEntries] = useState<AuditEntry[]>([]); const [limit, setLimit] = useState(50); const [error, setError] = useState('');
  const load = useCallback(async () => { try { setEntries((await api.audit(limit)).data); setError(''); } catch (cause) { setError(errorText(cause)); } }, [limit]);
  useEffect(() => { void load(); }, [load]);
  return <section className="page"><PageHeader eyebrow="Redacted operations" title="Audit Log" description="Request metadata and outcomes without tokens, secrets, passwords, or raw request bodies." action={<button className="secondary-button" onClick={load}>Refresh</button>} /><div className="toolbar"><label>Rows <select value={limit} onChange={e => setLimit(Number(e.target.value))}><option>25</option><option>50</option><option>100</option><option>200</option></select></label></div>{error && <ErrorBanner error={error} onRetry={load} />}{entries.length ? <div className="table-card"><table><thead><tr><th>Time</th><th>Action</th><th>Status</th><th>Source / Client / Job</th><th>Duration</th><th>Request ID</th></tr></thead><tbody>{entries.map(entry => <tr key={entry.id}><td>{formatTime(entry.created_at)}</td><td><strong>{entry.action}</strong>{entry.error_code && <code>{entry.error_code}</code>}</td><td><StatusBadge value={entry.status} /></td><td><code>{entry.source_id ?? entry.client_id ?? (entry.job_id ? `job:${entry.job_id}` : '—')}</code></td><td>{entry.params_summary?.duration_ms != null ? `${entry.params_summary.duration_ms} ms` : '—'}</td><td><code title={entry.request_id}>{shortId(entry.request_id)}</code></td></tr>)}</tbody></table></div> : <Empty>No audit entries yet.</Empty>}</section>;
}
