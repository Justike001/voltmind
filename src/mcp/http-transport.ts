/**
 * HTTP transport for `voltmind serve --http` (legacy bearer-auth path).
 *
 ...
 *   - Create/manage tokens with auth.ts (voltmind auth create/list/revoke)
 *   - No open OAuth, no client_credentials, no self-service tokens
 *
 * Hardening:
 *   - CORS default-deny: allowlist via VOLTMIND_HTTP_CORS_ORIGIN (comma-separated)
 ...
 *   - Body cap: 1 MiB default (VOLTMIND_HTTP_MAX_BODY_BYTES). Stream-counted, not buffered —
...
interface AuthResult {
   ...
   * Operators migrate to the full OAuth transport (voltmind serve --http)
...
/** Resolve client IP. Honors X-Forwarded-For only when VOLTMIND_HTTP_TRUST_PROXY=1. */
function resolveClientIp(req: Request, server: { requestIP: (r: Request) => { address: string } | null }): string {
  if (process.env.VOLTMIND_HTTP_TRUST_PROXY === '1') {
...
export async function startHttpTransport(opts: HttpTransportOptions) {
  ...
  const server = Bun.serve({
    ...
    async fetch(req, server) {
      ...
      if (!auth.ok) {
        ...
        return Response.json(
          { error: 'invalid_token', message: 'Bearer token required. Create one: voltmind auth create <name>' },
        ...
        return Response.json(
          {
            result: {
              ...
              serverInfo: { name: 'voltmind', version: VERSION },
              capabilities: { tools: {} },
            },
            jsonrpc: '2.0',
            id,
  ...
  console.error(`VoltMind HTTP MCP server running on port ${port}`);
  console.error(`  Health: http://localhost:${port}/health`);
  console.error(`  MCP:    http://localhost:${port}/mcp`);
  console.error(`  Auth:   Bearer token required (create with: voltmind auth create <name>)`);
  if (!corsAllowlist) {
    console.error('  CORS:   default-deny. Set VOLTMIND_HTTP_CORS_ORIGIN=https://your.app to allow browser clients.');
  } else {
    console.error(`  CORS:   allowlist = ${[...corsAllowlist].join(', ')}`);
  }
  console.error('');
  console.error('⚠️  Do NOT use open OAuth registration for remote MCP access.');
  console.error('   Tokens are managed via: voltmind auth create/list/revoke');
