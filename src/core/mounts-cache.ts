 ...
 * host's checked-in `skills/RESOLVER.md`, voltmind publishes an aggregated
 * `~/.voltmind/mounts-cache/RESOLVER.md` + `manifest.json` whenever mounts
...
function getMountsCacheDir(): string {
  return join(homedir(), '.voltmind', 'mounts-cache');
}
...
export function renderResolverMarkdown(composed: ComposedResolver): string {
  const lines: string[] = [];
  lines.push('# VoltMind Skill Resolver (aggregated)');
  lines.push('');
  lines.push('Auto-generated by `voltmind mounts add|remove|sync`. Do not edit by hand.');
...
/**
 * Write the aggregated cache to ~/.voltmind/mounts-cache/. Safe to call
...
export function writeMountsCache(
  ...
  // Unique tmp names per call so concurrent `voltmind mounts add|remove`
...
/** Remove the aggregated cache dir. Called by `voltmind mounts remove` and tests. */
