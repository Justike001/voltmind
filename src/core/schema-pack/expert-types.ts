...
// `expert_routing: true`. voltmind-base preserves person + company as
...
// the hardcoded DEFAULT_TYPES = ['person', 'company'] — which voltmind-
 ...
 * applicable" rather than falling back to voltmind-base defaults (the
 * pack made an explicit choice).
 *
 * For voltmind-base, this returns ['person', 'company'] (the pre-v0.38
...
export function expertTypesFromPackOrThrow(
  ...
  if (types.length === 0) {
    throw new Error(
      ...
      `expert_routing: true. \`voltmind whoknows\` and \`find_experts\` ` +
