...
export function validateSchemaPackTrustGate(
  ...
  if (ctx.remote !== false) {
    throw new SchemaPackTrustGateError(
      ...
      'Pass via voltmind.yml `schema:` section, ~/.voltmind/config.json `schema_pack`, ' +
      'VOLTMIND_SCHEMA_PACK env var, or `voltmind config set schema_pack <name>`. ' +
    ...
    if (scope.sourceIds.length === 1) {
      ...
      for (const sid of scope.sourceIds) {
        const res = resolveActivePackName({
          ...
          envVar: process.env.VOLTMIND_SCHEMA_PACK?.trim() || undefined,
          sourceId: sid,
