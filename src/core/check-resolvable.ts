 ...
 * 2. `voltmind doctor` — runtime health check with actionable agent guidance
...
const OVERLAP_WHITELIST = new Set([
  ...
  'maintain',         // router that selects its client / Host reference
 ...
 *
 *   Format 1 (table) — original voltmind shape:
...
function extractTriggers(skillContent: string): string[] {
  // SKILL.md files are frequently edited on Windows. Normalize line endings
  // before the lightweight scan so it agrees with the canonical parser.
...
export function checkResolvable(skillsDir: string): ResolvableReport {
  ...
  // `voltmind skillify scaffold`. Presence means a scaffolded skill
