/**
 * skillpack/scaffold.ts — `voltmind skillpack scaffold <name>`.
 *
 * One-time, additive copy of a bundled skill into a host workspace. The
 * file-copy primitive is shared with harvest (host→voltmind) via
 ...
 *      user owns it. To update, run `voltmind skillpack reference <name>`
 *      and decide.
...
export function runScaffold(opts: ScaffoldOptions): ScaffoldResult {
  ...
  // symlink check or path confinement (the scaffold source is voltmind's
  // own trusted bundle).
