 ...
 * doctor scan.
 *
 * File: `~/.voltmind/audit/backpressure-YYYY-Www.jsonl` (override dir via
 * `VOLTMIND_AUDIT_DIR` for container/sandbox deployments where `$HOME` is read-only).
 *
 * `voltmind jobs stats` will surface coalesce counts from this file in a v0.19.2+
...
/** Honors `VOLTMIND_AUDIT_DIR` for container/sandbox deployments. */
