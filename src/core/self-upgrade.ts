/**
 * Self-upgrade decision + state foundation (v0.42 self-upgrading-voltmind wave).
 *
 * Mirrors gstack's invocation-riding update mechanism for voltmind: a throttled
 * check that rides every `voltmind` invocation (CLI / MCP), emits a marker, and
 ...
 * always the hardcoded `voltmind upgrade` / `voltmind self-upgrade`).
 *
 * NO DB. The hot path runs before `connectEngine()` and thin clients have no
 * local DB, so all state is file-based under `~/.voltmind/` (honors VOLTMIND_HOME).
 */
...
export function updateCachePath(): string {
  return voltmindPath('last-update-check');
}

export function snoozePath(): string {
  return voltmindPath('update-snoozed');
}

export function justUpgradedPath(): string {
  return voltmindPath('just-upgraded-from');
}

/**
 * Record the version we just upgraded FROM, so the next `voltmind` invocation's
 ...
 * Takes a loosely-typed config so it doesn't pull the full VoltMindConfig type
 * onto the hot path. Env (`VOLTMIND_SELF_UPGRADE_MODE`) is the operator / CI
 * escape hatch.
 */
export function resolveSelfUpgradeMode(
  cfg: { self_upgrade?: { mode?: string } } | null | undefined,
): SelfUpgradeMode {
  const env = normalizeMode(process.env.VOLTMIND_SELF_UPGRADE_MODE);
  if (env) return env;
