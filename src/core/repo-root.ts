 ...
 *   - `env_explicit`                 — $VOLTMIND_SKILLS_DIR (operator override; v0.31.7)
 ...
 *   - `repo_root`                    — walked up from cwd, found voltmind repo
...
function resolveWorkspaceSkillsDir(
  ...
  // Preferred: workspace/skills with a resolver file inside it (voltmind-native).
 ...
 *   0. $VOLTMIND_SKILLS_DIR explicit operator override (any caller)
 *   1. $OPENCLAW_WORKSPACE when explicitly set (env > repo-root walk)
 *   2. ~/.openclaw/workspace/ (user's default OpenClaw deployment)
 *   3. findRepoRoot() walk from cwd (voltmind's own repo)
 *   4. ./skills fallback (dev scratch, fixtures)
 *
 * Tier 0 ($VOLTMIND_SKILLS_DIR) is safe for both read and write paths because
 ...
 * `export OPENCLAW_WORKSPACE=...; voltmind check-resolvable` run from
 * inside the voltmind repo silently shadowed the env var by walking up
 * to voltmind's own skills/. Explicit env should win. Unset env → behavior
 ...
 * a write-path install-path fallback would let `voltmind skillpack install`
 * from `~` silently target the bundled voltmind repo's skills/ instead of the
 * user's workspace.
 *
...
export function autoDetectSkillsDir(
  ...
  // 0. $VOLTMIND_SKILLS_DIR explicit operator override. Safe for all callers
  ...
  if (env.VOLTMIND_SKILLS_DIR) {
    const explicit = isAbsolute(env.VOLTMIND_SKILLS_DIR)
      ? env.VOLTMIND_SKILLS_DIR
      : resolvePath(startDir, env.VOLTMIND_SKILLS_DIR);
  ...
  //     && voltmind skillpack scaffold X` finds the agent repo, not an
 ...
 * walks up from this module's install location to find a voltmind repo root,
 * gated by `isGbrainRepoRoot` to avoid false-positive on unrelated repos.
 *
 * Use this from READ-ONLY callers only: `voltmind doctor`,
 * `voltmind check-resolvable`, `voltmind routing-eval`. Never from write paths.
 *
 ...
 * `voltmind skillpack install` from `~` silently target the bundled voltmind
...
export function autoDetectSkillsDirReadOnly(
  ...
  // that vendored voltmind in a subdir).
  try {
...
export const AUTO_DETECT_HINT = [
  `  1. --skills-dir flag`,
  `  2. $VOLTMIND_SKILLS_DIR (explicit operator override)`,
 ...
 * `autoDetectSkillsDirReadOnly` adds for `voltmind doctor` /
 * `voltmind check-resolvable` / `voltmind routing-eval`.
 */
export const AUTO_DETECT_HINT_READ_ONLY = [
  AUTO_DETECT_HINT,
  `  7. (read-only) walk up from voltmind's install path`,
].join('\n');
