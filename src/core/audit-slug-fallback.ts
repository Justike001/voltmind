 ...
 *
 * Writes info-severity rows to `~/.voltmind/audit/slug-fallback-YYYY-Www.jsonl`
 ...
 *
 * Why a separate JSONL instead of `~/.voltmind/sync-failures.jsonl`:
...
export interface SlugFallbackAuditEvent {
  ...
  severity: 'info';
  /** Stable code consumed by `voltmind doctor`'s slug_fallback_audit check. */
...
export function logSlugFallback(slug: string, sourcePath: string): void {
  ...
  process.stderr.write(`[voltmind] slug fallback: ${sourcePath} → ${slug} (frontmatter slug; path slugified empty)\n`);
  writer.log({
    slug,
 ...
 * latest week's JSONL. Used by `voltmind doctor`'s slug_fallback_audit check.
