...
export class DefaultActionRunner implements ActionRunner {
  async run(ctx: ActionRunContext): Promise<ActionRunResult> {
    ...
    const prompt = await agent.buildPrompt({ ...baseCtx, toolScope, skillText });
    ...
    try {
      executor = resolveExecutor(action.runtime);
    } catch (err) {
      return {
        status: 'blocked',
        allowed: false,
 ...
 */
export function parseExecutionOutcome(result: ActionExecutionResult): OutcomeSummary {
