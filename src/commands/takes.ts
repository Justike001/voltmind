...
async function resolveBrainDir(engine: BrainEngine | null, explicitDir: string | null): Promise<string> {
  ...
  console.error('No brain directory configured. Pass --dir <path> or run `voltmind init` first.');
  process.exit(1);
}
...
async function getPageId(engine: BrainEngine, slug: string): Promise<number> {
  ...
  if (!rows[0]) {
    console.error(`Page not found in brain: ${slug}. Run \`voltmind sync\` first.`);
    process.exit(1);
  }
  return rows[0].id;
}
...
async function cmdList(engine: BrainEngine, args: string[]): Promise<void> {
  const slug = args[0];
  if (!slug) {
    console.error('Usage: voltmind takes <slug> [--json]');
    process.exit(1);
  }
...
async function cmdSearch(engine: BrainEngine, args: string[]): Promise<void> {
  const query = args[0];
  if (!query) {
    console.error('Usage: voltmind takes search "<query>" [--who h] [--json]');
    process.exit(1);
  }
  const json = flagPresent(args, '--json');
...
async function cmdAdd(engine: BrainEngine, args: string[]): Promise<void> {
  const slug = args[0];
  if (!slug) {
    console.error('Usage: voltmind takes add <slug> --claim "..." --kind <k> --who <h> [--weight 0.5] [--source "..."] [--since YYYY-MM]');
    process.exit(1);
  }
...
async function cmdUpdate(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug || !rowNumStr) {
    console.error('Usage: voltmind takes update <slug> --row N [--weight 0.7] [--source "..."] [--since YYYY-MM]');
    process.exit(1);
  }
  ...
  await withPageLock(slug, async () => {
    ...
    if (!target) {
      console.warn(`[takes update] DB updated but row #${rowNum} not in markdown fence on disk; markdown may be out of sync. Run 'voltmind extract takes --slugs ${slug}' to reconcile.`);
      return;
    }
...
async function cmdSupersede(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  const rowNumStr = flagValue(args, '--row');
  if (!slug || !rowNumStr) {
    console.error('Usage: voltmind takes supersede <slug> --row N --claim "..." [--kind k] [--who h] [--weight 0.5] [--source "..."]');
    process.exit(1);
  }
  const rowNum = parseInt(rowNumStr, 10);
  const claim = flagValue(args, '--claim');
...
async function cmdResolve(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug || !rowNumStr || (!qualityStr && !outcomeStr)) {
    console.error('Usage: voltmind takes resolve <slug> --row N --quality correct|incorrect|partial|unresolvable [--evidence "..."] [--value N --unit usd|pct|count] [--by <slug>]');
    console.error('       (back-compat) voltmind takes resolve <slug> --row N --outcome true|false [...]');
    process.exit(1);
  }
  ...
  await withPageLock(slug, async () => {
    ...
    const target = parsed.takes.find(t => t.rowNum === rowNum);
    if (!target) {
      console.warn(`[takes resolve] DB updated but row #${rowNum} not in markdown fence; run 'voltmind extract takes --slugs ${slug}' to reconcile.`);
      return;
    }
...
export async function runTakes(engine: BrainEngine, args: string[]): Promise<void> {
  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: voltmind takes <subcommand> [options]
...
/**
 * v0.36.1.0 (TD4 / D30) — `voltmind takes revisit <slug>` opens $EDITOR on
...
async function cmdRevisit(_engine: BrainEngine, rest: string[]): Promise<void> {
  ...
  if (!repoPath) {
    process.stderr.write('No brain repo configured. Run `voltmind config set sync.repo_path /path/to/brain`.\n');
    process.exit(1);
  }
  ...
  const marker = '\n<!-- voltmind:revisit -->\n';
  if (!existing.includes('<!-- voltmind:revisit -->')) {
