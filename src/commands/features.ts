/**
 * voltmind features — Scan brain usage and recommend unused features.
 *
 * Usage:
 *   voltmind features [--json] [--auto-fix] [--help]
 */
...
function offersPath(): string {
  return join(process.env.HOME || '', '.voltmind', 'feature-offers.json');
}
...
async function scanFeatures(engine: BrainEngine): Promise<FeatureScanResult> {
  ...
  if (health.dead_links > 0) {
    recommendations.push({
      ...
      command: 'voltmind check-backlinks fix',
      auto_fixable: false,
    });
  }
  ...
  if (stats.page_count >= 3) {
    ...
    if (stats.link_count === 0 && stats.page_count > 5) {
      recommendations.push({
        ...
        command: 'voltmind extract links',
        auto_fixable: true,
      });
    }
    ...
    if (stats.timeline_entry_count === 0 && stats.page_count > 5) {
      recommendations.push({
        ...
        command: 'voltmind extract timeline',
        auto_fixable: true,
      });
    }
    ...
    if (unconfigured.length > 0) {
      recommendations.push({
        ...
        command: `voltmind integrations list`,
        auto_fixable: false,
      });
    }
    ...
    try {
      ...
      if (!syncRepo) {
        recommendations.push({
          ...
          command: 'voltmind sync --repo <path>',
          auto_fixable: false,
        });
      }
    } catch { /* skip */ }
...
export async function featuresTeaserForDoctor(engine: BrainEngine): Promise<string | null> {
  try {
    ...
    return `Tip: ${parts.join(', ')}. Run 'voltmind features' to fix.`;
  } catch {
    return null;
  }
}
