...
export async function runServe(
  ...
  // that used `voltmind auth create` keep working unchanged).
  ...
  if (isHttp) {
    ...
    // VOLTMIND_ADMIN_BOOTSTRAP_TOKEN for production deployments that don't
  ...
  console.error('Starting VoltMind MCP server (stdio)...');
...
function installStdioLifecycle(
  ...
  const beginShutdown = (reason: string): void => {
    ...
    deps.log(`VoltMind MCP server: graceful exit (${reason})`);
    ...
    const deadline = setTimeout(() => {
      deps.log(
        `VoltMind MCP server: cleanup deadline (${CLEANUP_DEADLINE_MS}ms) exceeded — forcing exit`,
      );
      deps.exit(0);
    }, CLEANUP_DEADLINE_MS);
    deadline.unref?.();
    ...
    Promise.resolve()
      ...
      .catch((err: unknown) => {
        ...
        deps.log(`VoltMind MCP server: cleanup error: ${msg}`);
      })
      .finally(() => {
  ...
  // Skip when stdin is a TTY: interactive `voltmind serve` use shouldn't
  ...
  if (initialParentPid !== 1) {
    if (!deps.probeWatchdog()) {
      deps.log(
        '[voltmind serve] watchdog disabled: ps unavailable, parent-death detection unavailable — child will rely on stdin EOF / signals only',
      );
    } else {
  ...
  if (idleTimeoutSec > 0) {
    ...
    deps.log(`VoltMind MCP server: stdio idle timeout = ${idleTimeoutSec}s`);
  }
}
