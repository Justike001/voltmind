...
/**
 * `voltmind call <tool> <json>` — trusted local op-dispatch surface.
 *
 ...
 * 6-tier chain (--source > VOLTMIND_SOURCE > .voltmind-source dotfile > path-match
...
export async function runCall(engine: BrainEngine, args: string[]) {
  ...
  // tokens to keep the existing `voltmind call <tool> <json>` shape readable,
  ...
  if (!tool) {
    console.error("Usage: voltmind call [--source <id>] <tool> '<json>'");
    process.exit(1);
  }
