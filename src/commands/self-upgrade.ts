 ...
 * command. The action is always the hardcoded `voltmind upgrade` — never
...
export async function runSelfUpgrade(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'Usage: voltmind self-upgrade [--check-only] [--force] [--json]\n\n' +
        'Check for and apply voltmind updates. The shared entry point used by the\n' +
        'CLI startup marker and agent-driven update flows.\n\n' +
  ...
  if (checkOnly) {
    ...
    if (json) {
      ...
      console.log(`Update available: ${VERSION} -> ${latest}. Run: voltmind self-upgrade`);
      if (changelogDiff) {
        console.log('\nWhat changed:\n');
