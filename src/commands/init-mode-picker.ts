 ...
 *
 * Runs as a phase inside `voltmind init` AFTER `engine.initSchema()` so DB
 ...
 * `balanced` and prints the one-line hint pointing at `voltmind config set
...
The "cost" isn't voltmind itself — it's the downstream agent's input cost
reading the retrieved chunks back into its context window. voltmind's own
...
You can change this any time with: voltmind config set search.mode <mode>
Per-knob tuning + recommendation engine ships at: voltmind search tune
`;
...
export async function runModePicker(
  ...
  if (!process.stdin.isTTY) {
    ...
    console.log('');
    console.log('═══════════════════════════════════════════════════════════════');
    console.log(`[voltmind] search mode tentatively set to: ${rec.mode}`);
    console.log(`[voltmind] Reason: ${rec.reason}`);
    console.log('═══════════════════════════════════════════════════════════════');
    console.log('');
    ...
    console.log('[AGENT] v0.31.x retrieval shape. Run `voltmind config set search.mode');
    ...
    console.log('');
    console.log('To change later: voltmind config set search.mode <mode>');
    console.log('To see what is running: voltmind search modes');
    console.log('');
    return rec.mode;
  }
  ...
  try {
    ...
    console.error(`[voltmind] WARN: failed to persist search.mode (${(err as Error).message ?? 'unknown'}). Defaulting to ${picked} at search-time.`);
  }
