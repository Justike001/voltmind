/**
 * voltmind eval gate — fail CI on retrieval regressions OR correctness drops (v0.41).
 *
 ...
 * the voltmind-version-drift bug class for source-tree CI runs.
 *
...
function printHelp(): void {
  console.log(`voltmind eval gate — fail CI on retrieval regressions or correctness drops

Usage:
  voltmind eval gate --baseline X.baseline.ndjson [flags]      # regression-only
  voltmind eval gate --qrels Y.qrels.json [flags]              # correctness-only
  voltmind eval gate --baseline X --qrels Y [flags]            # both required

Required (at least one):
  --baseline FILE              Baseline NDJSON from \`voltmind bench publish\`
