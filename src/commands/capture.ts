/**
 * voltmind capture — the single human-facing entrypoint for getting content
 ...
 *
 *   voltmind capture "thought to remember"
 *   voltmind capture --file ./notes/2026-05-20.md
 *   echo "from stdin" | voltmind capture --stdin
 *   voltmind capture "..." --slug inbox/specific
 *   voltmind capture "..." --quiet           # slug-only output for pipelines
 *
 * Behavior:
 *   - Local install: writes to ~/.voltmind/inbox/<slug>.md OR routes through
...
const HELP = `Usage: voltmind capture [content] [options]
...
Modes (mutually exclusive — first match wins):
  voltmind capture "thought"          inline content
  voltmind capture --file PATH        read content from a file
  voltmind capture --stdin            read content from stdin (piped)
...
Options:
  ...
  --source ID          Multi-source brains: write under a non-default source.
                       Resolution: --source flag > VOLTMIND_SOURCE env >
                       .voltmind-source dotfile (walk-up) > local_path >
...
Examples:
  voltmind capture "remember to follow up on the X deal"
  echo "from a pipe" | voltmind capture --stdin
  voltmind capture --file ./notes/today.md --slug daily/2026-05-20
  JOB=$(voltmind capture "..." --quiet)
`;
...
export async function runCapture(engine: BrainEngine | null, args: string[]): Promise<void> {
  ...
  if (parsed.source && isThinClient(cfg)) {
    console.error(`voltmind capture: --source is not supported on thin-client installs.`);
    console.error(`Server-side OAuth client registration determines source scope.`);
    console.error(`On the server, run:`);
    console.error(`  voltmind auth register-client <name> --source ${parsed.source} --scopes "read write"`);
    process.exit(1);
  }
  ...
  if (parsed.stdin) {
    ...
    try {
      ...
      console.error(
        `voltmind capture: failed to read ${parsed.filePath}: ${e instanceof Error ? e.message : String(e)}`,
      );
      process.exit(1);
    }
    ...
    console.error('voltmind capture: provide content positionally, --file PATH, or --stdin');
    console.error('Run `voltmind capture --help` for examples.');
    process.exit(1);
  }
  ...
  if (nullByteOffset !== -1) {
    console.error(
      `voltmind capture: refusing to capture binary content from ${inputLabel}\n` +
  ...
  if (normalizedBody.length === 0) {
    console.error('voltmind capture: refusing to capture empty content');
    process.exit(1);
  }
  ...
  if (!isThinClient(cfg) && engine) {
    try {
      ...
      console.error(`voltmind capture: ${e instanceof Error ? e.message : String(e)}`);
      process.exit(1);
    }
  }
  ...
  if (isThinClient(cfg)) {
    ...
    try {
      ...
      if (hint) {
        console.error(`voltmind capture: ${hint}`);
      } else if (e instanceof RemoteMcpError) {
        console.error(`voltmind capture: remote put_page failed: ${e.message}`);
        console.error('Run `voltmind remote doctor` to diagnose the connection.');
      } else {
        console.error(
          `voltmind capture: remote put_page failed: ${e instanceof Error ? e.message : String(e)}`,
        );
        console.error('Run `voltmind remote doctor` to diagnose the connection.');
      }
      process.exit(1);
  ...
  if (!engine) {
    console.error('voltmind capture: engine not connected');
    process.exit(1);
  }
  const putPageOp = operations.find((o) => o.name === 'put_page');
  if (!putPageOp) {
    console.error('voltmind capture: put_page operation missing (voltmind build issue)');
    process.exit(1);
  }
  ...
  try {
    ...
    if (hint) {
      ...
      console.error(
        `voltmind capture: put_page failed: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
    process.exit(1);
  }
