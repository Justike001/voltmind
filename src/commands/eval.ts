/**
 * voltmind eval — Retrieval Evaluation Command
 *
 ...
 *
 * Usage:
 *   voltmind eval --qrels <path|json>
 *   voltmind eval --qrels <path> --config-a <path|json> --config-b <path|json>
 *   voltmind eval --qrels <path> --strategy hybrid --rrf-k 30 --k 5
 */
...
export async function runEvalCommand(engine: BrainEngine, args: string[]): Promise<void> {
  // v0.25.0 — sub-subcommand dispatch. Bare `voltmind eval --qrels ...`
...
function printSingleTable(report: EvalReport): void {
  ...
  console.log(`\nvoltmind eval - ${queries.length} quer${queries.length === 1 ? 'y' : 'ies'} · strategy: ${label} · k=${k}\n`);
...
function printABTable(reportA: EvalReport, reportB: EvalReport, k: number): void {
  ...
  console.log(`\nvoltmind eval - ${n} quer${n === 1 ? 'y' : 'ies'} · A/B comparison · k=${k}\n`);
...
function printHelp(): void {
  console.log(`
voltmind eval — measure and compare retrieval quality

USAGE
  voltmind eval --qrels <path>
  voltmind eval --qrels <path> --config-a <path> --config-b <path>
...
EXAMPLES
  voltmind eval --qrels ./my-queries.json
  voltmind eval --qrels ./qrels.json --strategy keyword
  voltmind eval --qrels ./qrels.json --rrf-k 30
  voltmind eval --qrels ./qrels.json --config-a baseline.json --config-b experiment.json
`.trim());
}
