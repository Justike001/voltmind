 ...
 * is invoked, so the user's ~/.voltmind brain is never opened. Tests stub
 ...
 * "voltmind + Haiku-preprocess pipeline" vs "voltmind alone", not directly
...
function printHelp(): void {
  process.stderr.write(
    `voltmind eval longmemeval <dataset.jsonl> [options]\n\n` +
    `Run the LongMemEval benchmark against voltmind's hybrid retrieval. Spins up an\n` +
...
export interface RunOpts {
   ...
   * one inside this call. Production callers (the voltmind CLI) leave this
...
export async function runEvalLongMemEval(args: string[], runOpts: RunOpts = {}): Promise<void> {
  ...
  const work = async (engine: PGLiteEngine): Promise<void> => {
    ...
    for (const q of questions) {
      ...
      if (process.env.VOLTMIND_LME_DEBUG === '1') {
