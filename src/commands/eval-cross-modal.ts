/**
 * voltmind eval cross-modal — multi-model quality gate (v0.27.x).
 *
 ...
 * before `voltmind init`. Receipts are bound to (slug, SKILL.md sha-8) so
 * `voltmind skillify check` can detect stale audits (T10=A).
 *
...
import { voltmindPath, loadConfig } from '../core/config.ts';
...
const HELP = `voltmind eval cross-modal — multi-model quality gate

USAGE:
  voltmind eval cross-modal --task "<description>" --output <path-or-skill-slug> [flags]
  voltmind eval cross-modal --batch <jsonl> [--limit N] [--output <receipt-path>] [flags]   (v0.40.1.0 Track D)
...
REQUIRED (batch mode, v0.40.1.0 Track D / T3):
  --batch <jsonl>          LongMemEval-shape JSONL (output of \`voltmind eval
...
BATCH FLAGS:
  ...
  --output PATH            Where to write the SUMMARY receipt (NOT where to
                           ...
                           ~/.voltmind/eval-receipts/cross-modal-batch-<sha8>.json
...
FLAGS:
  ...
  --receipt-dir <path>     Default: voltmindPath('eval-receipts').
...
CONFIGURATION:
  Models resolve via the voltmind AI gateway. Configure with:
    voltmind providers test            # see what's configured
    voltmind config                    # set keys
  Or set env vars: OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY,
  TOGETHER_API_KEY, etc. The gateway reads from \`~/.voltmind/config.json\` plus
  process.env.

EXAMPLES:
  voltmind eval cross-modal \\
...
/**
 * Configure the AI gateway from `~/.voltmind/config.json` + process.env.
 *
...
export async function runEvalCrossModal(args: string[], opts: RunCrossModalOpts = {}): Promise<number> {
  ...
  const receiptDir = parsed.receiptDir ?? voltmindPath('eval-receipts');
  ...
  if (!isAvailable('chat')) {
    process.stderr.write(
      'Error: AI gateway has no usable chat provider. ' +
        ...
        'in your shell or run `voltmind config` to set keys.\n',
    );
    return 1;
  }
...
// at end of run (per D10 — keeps ~/.voltmind/eval-receipts/ clean).
// ---------------------------------------------------------------------------
 ...
 * `voltmind eval longmemeval`. Carries `question`+`question_type` and an
...
function readBatchRows(path: string): BatchReadResult {
  ...
  for (const line of raw.split('\n')) {
    ...
    // `voltmind eval longmemeval` carry an `error` field and an empty/missing
...
async function runBatchMode(parsed: ParsedArgs, opts: RunCrossModalOpts): Promise<number> {
  ...
  // end of the batch (per D10 — keeps ~/.voltmind/eval-receipts/ clean).
  const batchTempDir = mkdtempSync(join(tmpdir(), 'voltmind-batch-receipts-'));
  const runEvalFn = opts.runEval ?? runEval;
  ...
  try {
    ...
    const summaryPath = parsed.output ??
      join(voltmindPath('eval-receipts'), `cross-modal-batch-${batchSha8(summary)}.json`);
