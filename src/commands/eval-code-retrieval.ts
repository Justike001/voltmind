/**
 * v0.34 pre-w0 — `voltmind eval code-retrieval` CLI.
 *
 * Subcommands:
...
function parseArgs(args: string[]): ParsedArgs {
  ...
  while (i < args.length) {
    ...
    } else if (a === '--corpus') out.corpus = args[++i] ?? 'voltmind';
...
voltmind eval code-retrieval — v0.34 retrieval baseline + gate
...
USAGE:
  voltmind eval code-retrieval --baseline [--corpus NAME] [--save PATH] [--json]
  voltmind eval code-retrieval --with-code-intel [--corpus NAME] [--save PATH] [--json]
  voltmind eval code-retrieval --compare BASELINE.json WITH_CODE_INTEL.json [--json]
...
OPTIONS:
  ...
  --corpus NAME            Brain corpus to query (default: voltmind)
...
PRE-V0.34 BASELINE WORKFLOW (the assignment):
  ...
  voltmind eval code-retrieval --baseline --save /tmp/baseline-1.json
  voltmind eval code-retrieval --baseline --save /tmp/baseline-2.json
  voltmind eval code-retrieval --baseline --save /tmp/baseline-3.json

V0.34 SHIP GATE:
  # After v0.34 ships, capture the with-code-intel run and compare
  voltmind eval code-retrieval --with-code-intel --save /tmp/v034.json
  voltmind eval code-retrieval --compare /tmp/baseline-1.json /tmp/v034.json
`);
}
