/**
 * `voltmind apply-migrations` — migration runner CLI.
 *
 * Reads ~/.voltmind/migrations/completed.jsonl, diffs against the TS migration
 ...
 *
 * Invoked from:
 *   - `voltmind upgrade` → runPostUpgrade() tail (Lane A-5)
...
function printHelp(): void {
  console.log(`voltmind apply-migrations — run pending migration orchestrators.

Usage:
  voltmind apply-migrations                Run all pending migrations interactively.
  voltmind apply-migrations --yes          Non-interactive; uses default mode (pain_triggered).
  voltmind apply-migrations --dry-run      Print the plan; take no action.
  voltmind apply-migrations --list         Show applied + pending migrations.
  voltmind apply-migrations --migration vX.Y.Z
                                         Force-run a specific migration by version.
  voltmind apply-migrations --force-retry vX.Y.Z
                                         Clear a wedged migration (3+ consecutive
                                         partials). Writes a 'retry' marker so the
                                         next run treats it as fresh.
  voltmind apply-migrations --force-orchestrator
                                         Reset every wedged orchestrator migration
                                         in one shot (writes 'retry' for each).
  voltmind apply-migrations --force-schema
                                         Reset schema-version drift; re-runs
                                         runMigrations from current config.version.
  voltmind apply-migrations --force        (alias --force-all) Apply both
                                         --force-orchestrator and --force-schema.
  voltmind apply-migrations --skip-verify  Bypass post-condition verify hooks on
...
function printList(plan: Plan, installed: string): void {
  console.log(`Installed voltmind version: ${installed}\n`);
  ...
  if (needsWork === 0) {
    ...
    console.log(`${needsWork} migration(s) need action. Run \`voltmind apply-migrations --yes\` to apply.`);
  }
}

function printDryRun(plan: Plan, installed: string): void {
  console.log(`Dry run — installed voltmind version: ${installed}`);
  console.log('');
...
export async function runApplyMigrations(args: string[]): Promise<void> {
  ...
  // First-install guard (postinstall hook calls us even on `bun add voltmind`
  // before the user has run `voltmind init`). No config = no brain = nothing
  ...
  if (!loadConfig()) {
    if (cli.list) console.log('No brain configured. Run `voltmind init` to set one up.');
    else if (cli.dryRun) console.log('No brain configured (run `voltmind init` first). Nothing to migrate.');
    return;
  }

  // Bug 3 — --force-retry: write an explicit reset marker for a wedged
  // migration, then return. User re-runs `voltmind apply-migrations --yes`
  // to actually re-attempt.
  if (cli.forceRetry) {
    const target = migrations.find(m => m.version === cli.forceRetry);
    if (!target) {
      console.error(`No migration registered with version "${cli.forceRetry}". Run \`voltmind apply-migrations --list\`.`);
      process.exit(2);
    }
    appendCompletedMigration({ version: cli.forceRetry, status: 'retry' });
    console.log(`Wrote 'retry' marker for v${cli.forceRetry}. Run \`voltmind apply-migrations --yes\` to re-attempt.`);
    return;
  }
  ...
  // `voltmind apply-migrations --yes` to actually re-attempt.
  if (cli.forceOrchestrator || cli.forceAll) {
    ...
    if (resetCount === 0) {
      ...
      console.log(`\nReset ${resetCount} wedged orchestrator migration(s). Run \`voltmind apply-migrations --yes\` to re-attempt.`);
    }
  ...
  try {
    ...
    if (cfg) {
      ...
      // phase spawns `voltmind init --migrate-only` as a subprocess (the
      ...
      if (!skipPreflight) {
        ...
        if (schemaVer < LATEST_VERSION) {
          console.warn(
            ...
            `   To run them now: voltmind init --migrate-only\n`,
          );
        }
      }
    }
  ...
  if (plan.wedged.length > 0) {
    for (const m of plan.wedged) {
      console.error(
        ...
        `Check ~/.voltmind/upgrade-errors.jsonl for the last failure reasons, fix the underlying issue, then run:\n` +
        `  voltmind apply-migrations --force-retry ${m.version}\n` +
  ...
  if (cli.specificMigration && plan.applied.length + plan.partial.length + plan.pending.length + plan.skippedFuture.length === 0) {
    console.error(`No migration registered with version "${cli.specificMigration}". Run \`voltmind apply-migrations --list\` to see registered versions.`);
    process.exit(2);
  }
  ...
  for (const m of toRun) {
    ...
    try {
      ...
      if (result.status === 'partial') {
        console.log(`Migration v${m.version} finished as PARTIAL. Re-run \`voltmind apply-migrations --yes\` after resolving any pending host-work items.`);
      } else {
