 ...
 *
 *   A. Schema — `voltmind init --migrate-only` triggers migrate.ts v11 which
 *               adds link_source + origin_page_id + origin_field columns,
 *               swaps the unique constraint to include them, and creates
 *               new indexes.
 *   B. Backfill — `voltmind extract links --source db --include-frontmatter`
 ...
 *   D. Record — append to ~/.voltmind/completed.jsonl.
 *
...
//
// Shell out to the canonical `voltmind` shim on PATH (`/usr/local/bin/voltmind`
...
function phaseASchema(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    execSync('voltmind init --migrate-only', { stdio: 'inherit', timeout: 600_000, env: process.env });
...
function phaseBBackfill(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    execSync('voltmind extract links --source db --include-frontmatter', {
      stdio: 'inherit',
...
function phaseCVerify(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    // to `voltmind graph-query` as a smoke test: extract one random page and
    ...
    const out = execSync('voltmind call get_stats', {
...
export const v0_13_0: Migration = {
  ...
  featurePitch: {
    ...
    description:
      ...
      '`voltmind graph`. Direction semantics respect subject-of-verb (Pedro → meeting, ' +
