/**
 * voltmind eval prune — delete old eval_candidates rows (v0.21.0).
 *
 * Retention is unlimited by default (matches ingest_log precedent).
 * This command is the explicit cleanup; pairs with `voltmind eval export`
 * (snapshot first, then prune if you want to reset).
 *
 * Usage:
 *   voltmind eval prune --older-than 30d
 *   voltmind eval prune --older-than 1h --dry-run
 */
...
function printHelp(): void {
  console.error(`voltmind eval prune — delete old eval_candidates rows

USAGE:
  voltmind eval prune --older-than DUR [--dry-run]
...
EXAMPLES:
  voltmind eval prune --older-than 30d
  voltmind eval prune --older-than 90d --dry-run
`);
}
