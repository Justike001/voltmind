/**
 * v0.32.3 — `voltmind search` CLI surface.
 *
 * Three sub-subcommands, mirroring `voltmind models` (v0.31.12) for shape
 * consistency:
 *
 *   voltmind search modes [--json]
 ...
 *
 *   voltmind search tune [--apply] [--json]
...
async function buildModesReport(engine: BrainEngine): Promise<SearchModesReport> {
  ...
  const knobs: Array<keyof ModeBundle> = [
    ...
    'expansion',
    'searchLimit',
    // v0.35.6.0 — floor-ratio surfaced in `voltmind search modes` dashboard
...
function formatModesText(report: SearchModesReport): string {
  ...
  lines.push('');
  lines.push('Mode bundles (frozen — set via `voltmind config set search.mode <mode>`):');
...
async function runModesSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (reset || dryRunSource) {
    ...
    for (const k of toRemove) console.log(`  - ${k}`);
    console.log(`Mode bundle is now the only voice. Verify with: voltmind search modes`);
    return;
  }
...
async function runStatsSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (stats.total_calls === 0) {
    console.log('');
    console.log('No telemetry recorded yet. Run a few `voltmind query` calls and re-check.');
...
async function runTuneSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (stats.total_calls < 20) {
    if (json) {
      console.log(JSON.stringify({
        ...
        recommendations: [],
        message: 'Not enough search activity in the last 7 days to tune. Run `voltmind search stats` after some real usage.',
      }, null, 2));
      return;
    }
    ...
    console.log('Run a few `voltmind query` calls, then re-run `voltmind search tune`.');
    return;
  }
  ...
  if (resolved.resolved_mode === 'conservative' && stats.total_calls > 0) {
    ...
    if (dropPctPerCall > 2) {
      recs.push({
        ...
        apply_command: 'voltmind config set search.mode balanced',
      });
    }
  }
  ...
  if (stats.cache_hit_rate > 0.85 && stats.cache_hits + stats.cache_misses > 50) {
    recs.push({
      ...
      apply_command: 'voltmind config set search.cache.similarity_threshold 0.94',
    });
  }
  ...
  if (!resolved.cache_enabled && stats.total_calls > 5) {
    recs.push({
      ...
      apply_command: 'voltmind config unset search.cache.enabled',
    });
  }
  ...
  if (apply) {
    ...
    console.log('Run `voltmind search tune --apply` to apply these changes automatically.');
  }
}
...
async function maybeApplyRecommendation(engine: BrainEngine, r: TuneRecommendation): Promise<void> {
  ...
  if (parts[0] !== 'voltmind' || parts[1] !== 'config') return;
...
function buildRevertCommand(r: TuneRecommendation): string {
  const parts = r.apply_command.split(/\s+/);
  if (parts[2] === 'set') {
    return `voltmind config set ${parts[3]} ${String(r.current)}`;
...
const USAGE = `Usage: voltmind search <modes|stats|tune> [flags]
...
Examples:
  voltmind search modes
  voltmind search modes --reset
  voltmind search stats --days 30 --json
  voltmind search tune
  voltmind search tune --apply
`;
...
/**
 * `voltmind search modes` is read-only — no DB connection strictly required
 ...
 * this export is here so future no-engine modes (e.g. `voltmind search --help`
