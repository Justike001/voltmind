/**
 * voltmind skillify <scaffold|check> — W4 CLI namespace.
 *
...
const HELP_TOP = `voltmind skillify <subcommand> [options]
...
Run \`voltmind skillify <subcommand> --help\` for per-subcommand options.
`;
...
const HELP_SCAFFOLD = `voltmind skillify scaffold <name> [options]
...
\`voltmind check-resolvable --strict\` fails if any skill still has the
...
export async function runSkillifyScaffold(args: string[]): Promise<void> {
  ...
  if (flags.json) {
    ...
    console.log(`  3. voltmind skillify check skills/${flags.name}/scripts/${flags.name}.mjs`);
    console.log(`  4. voltmind check-resolvable`);
  }
}

// ---------------------------------------------------------------------------
// `voltmind skillify check` — delegates to scripts/skillify-check.ts via same
