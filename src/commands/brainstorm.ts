/**
 * voltmind brainstorm — bisociation-style idea generation grounded in your
 * own notes.
 *
...
const BRAINSTORM_HELP = `Usage: voltmind brainstorm <question> [options]
...
Examples:
  voltmind brainstorm "why are AI coding tools converging on the same UX?"
  voltmind brainstorm "what's the real bottleneck on lab automation" --json

Cost: ~$0.05-0.15 per run. Set VOLTMIND_NO_BRAINSTORM_PREVIEW=1 or pass --yes
to skip the TTY grace window in scripted callers.

See also: voltmind lsd — Lateral Synaptic Drift, the inverted-judge variant
that prefers forgotten pages and rejects ideas that are "too obvious."
`;

const LSD_HELP = `Usage: voltmind lsd <question> [options]

LSD = Lateral Synaptic Drift. Same bisociation engine as \`voltmind brainstorm\`
...
Examples:
  voltmind lsd "why are AI coding tools converging on the same UX?"
  voltmind lsd "the unspoken assumption in venture pricing" --save

Cost: ~$0.20-0.40 per run.

See also: voltmind brainstorm — the sober, cite-heavy default variant.
`;
...
async function runBrainstormCli(
  ...
  if (parsed.error) {
    console.error(`voltmind ${profile.label}: ${parsed.error}`);
    console.error(help);
    process.exit(2);
    return;
  }
  ...
  if (!parsed.question || parsed.question.trim().length === 0) {
    console.error(`voltmind ${profile.label}: question required`);
    console.error(help);
    process.exit(2);
    return;
  }
  ...
  const skipPreview = parsed.yes || process.env.VOLTMIND_NO_BRAINSTORM_PREVIEW === '1';
  ...
  if (shouldSave) {
    ...
    try {
      ...
      console.error(`voltmind ${profile.label}: save failed: ${msg}`);
    }
  }
}
