/**
 * `voltmind cache` \u2014 manage the semantic query cache (v0.32.x search-lite).
 *
 * Subcommands:
 *   voltmind cache stats   \u2014 print row/hit counts and freshness breakdown.
 *   voltmind cache clear   \u2014 wipe all cache rows.
 *   voltmind cache prune   \u2014 delete only stale (past-TTL) rows.
 *
...
Usage:
  voltmind cache stats        Print cache row counts, hit counts, freshness.
  voltmind cache clear        Wipe ALL cache rows. Requires --yes.
  voltmind cache prune        Delete only stale (past-TTL) rows.
...
export async function runCache(args: string[]): Promise<void> {
  ...
  if (!config) {
    // eslint-disable-next-line no-console
    console.error('voltmind cache: no brain configured. Run `voltmind init` first.');
    process.exit(1);
  }
  ...
  try {
    ...
    if (sub === 'clear') {
      ...
      if (!yes) {
        // eslint-disable-next-line no-console
        console.error('voltmind cache clear: refusing to wipe without --yes flag.');
        process.exit(1);
      }
    ...
    // eslint-disable-next-line no-console
    console.error(`voltmind cache: unknown subcommand "${sub}". See \`voltmind cache --help\`.`);
    process.exit(1);
  } finally {
