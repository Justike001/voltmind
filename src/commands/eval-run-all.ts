/**
 * v0.32.3 — `voltmind eval run-all` orchestrator.
 *
 * Sweeps every requested mode × suite combination and writes per-run
 * results to `<repo>/.voltmind-evals/eval-results.jsonl` [CDX-23]. Personal
 * brain (~/.voltmind) is never touched; the repo's git history is the
 * audit trail.
 *
...
function printHelp(): void {
  process.stderr.write(
    `voltmind eval run-all [flags]\n\n` +
    `Sweeps every requested search-lite mode × eval suite. Writes per-run results to\n` +
    `<repo>/.voltmind-evals/eval-results.jsonl. Personal brain is never touched.\n\n` +
    `Flags:\n` +
    ...
    `  --output DIR                  Override .voltmind-evals/ location.\n` +
...
function evalResultsPath(repoRoot: string, outputDirOverride?: string): string {
  ...
  return join(repoRoot, '.voltmind-evals', 'eval-results.jsonl');
}
...
export async function runEvalRunAll(_engine: BrainEngine | null, args: string[]): Promise<void> {
  ...
  // responsibility today — `voltmind eval run-all` is the orchestrator's
  // shape + cost guard + audit trail. The per-suite per-mode calls land
  // as a follow-up: each suite's CLI is already exposed (voltmind eval
  // longmemeval --mode X, voltmind eval replay --mode X), so wiring them
