/**
 * v0.28: `voltmind think <question>` CLI.
 *
...
export async function runThinkCli(engine: BrainEngine, args: string[]): Promise<void> {
  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: voltmind think "<question>" [options]
...
`);
  ...
  if (!question) {
    console.error('Missing question. Try: voltmind think "What do we know about acme-example?"');
    process.exit(1);
  }
