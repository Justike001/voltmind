...
import { voltmindPath, loadConfig, configPath } from '../core/config.ts';
...
export async function runSchema(args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    default:
      ...
      console.error('Run `voltmind schema --help` for available commands.');
      process.exit(2);
  }
}

function printHelp(): void {
  console.log(`voltmind schema — active schema pack management

Inspection:
  active                  Show resolved pack + which tier provided it
  list                    List installed packs (bundled + ~/.voltmind/schema-packs/)
...
Activation:
  use <pack>              Activate pack (writes ~/.voltmind/config.json schema_pack)
  downgrade [--to <pack>] Restore the previous active pack
  reload [--pack <name>]  Flush the in-process pack cache; --pack scopes

Authoring (v0.40.6.0):
  init <name>             Scaffold a new pack (extends voltmind-base)
...
function runList(_args: string[]): void {
  ...
  if (installed.length > 0) {
    console.log('\nInstalled packs (~/.voltmind/schema-packs/):');
    for (const name of installed) console.log(`  ${name}`);
  } else {
    console.log('\nNo user-installed packs (~/.voltmind/schema-packs/ empty or missing).');
  }
}

async function runShow(args: string[]): Promise<void> {
  // v0.39 T18 — `voltmind schema show --as-filing-rules` emits the JSON
  ...
  if (packName) {
    ...
    if (!path) {
      ...
      console.error('Run `voltmind schema list` to see available packs.');
      process.exit(1);
    }
  ...
  if (asFilingRules) {
    ...
    const filingRules = {
      schema_version: 1,
      source: 'voltmind schema show --as-filing-rules',
...
function runUse(args: string[]): void {
  const packName = args[0];
  if (!packName) {
    console.error('Usage: voltmind schema use <pack-name>');
    process.exit(2);
  }
  const path = packPathByName(packName);
  if (!path) {
  ...
  // Write to file-plane config (~/.voltmind/config.json schema_pack field).
  ...
  console.log(`\nRun \`voltmind schema active\` to verify resolution.`);
}
...
async function runDetectCmd(args: string[]): Promise<void> {
  ...
  console.log('');
  console.log('Next: voltmind schema review-candidates  (decide promote / rename / ignore)');
  console.log('      voltmind schema suggest             (LLM refinement on this candidate)');
}
...
async function runReviewCandidatesCmd(args: string[]): Promise<void> {
  ...
  console.log(`Audit history (cross-reference): ~/.voltmind/audit/schema-candidates-*.jsonl`);
  console.log('');
  if (result.applied) {
...
async function runInitCmd(args: string[]): Promise<void> {
  ...
  if (!name) {
    console.error('Usage: voltmind schema init <pack-name>  (experimental)');
    process.exit(2);
  }
  const baseDir = voltmindPath('schema-packs', name);
  ...
  const stub = {
    api_version: 'voltmind-schema-pack-v1' as const,
    name,
    version: '0.0.1',
...
`;
  ...
  console.log(`Next: edit pack.yaml, then run \`voltmind schema validate ${name}\` and \`voltmind schema use ${name}\`.`);
}
...
async function runForkCmd(args: string[]): Promise<void> {
  ...
  if (!from || !to) {
    console.error('Usage: voltmind schema fork <source-pack> <new-name>  (experimental)');
    process.exit(2);
  }
  ...
  const toDir = voltmindPath('schema-packs', to);
  if (existsSync(toDir)) {
...
async function runEditCmd(args: string[]): Promise<void> {
  ...
  const name = positional[0];
  if (!name) {
    console.error('Usage: voltmind schema edit <pack-name>  (experimental)');
    process.exit(2);
  }
  ...
  console.log(`Open it in your editor; then run \`voltmind schema validate ${name}\`.`);
}
...
async function runDiffCmd(args: string[]): Promise<void> {
  ...
  if (!a || !b) {
    console.error('Usage: voltmind schema diff <pack-a> <pack-b>  (experimental)');
    process.exit(2);
  }
...
async function runExplainCmd(args: string[]): Promise<void> {
  ...
  if (!typeName) {
    console.error('Usage: voltmind schema explain <type-name>  (experimental)');
    process.exit(2);
  }
  const cfg = loadConfig();
  const pack = await loadActivePack({ cfg, remote: false });
...
async function runDowngradeCmd(args: string[]): Promise<void> {
  ...
  const target = positional.includes('--to')
    ...
    : undefined;
  // Find the previous pack from ~/.voltmind/schema-pack-history.jsonl OR honor --to <pack>.
  const historyPath = voltmindPath('schema-pack-history.jsonl');
  ...
  console.log('Run `voltmind schema active` to verify. Note: this command restores CONFIG only.');
...
async function runSyncCmd(args: string[]): Promise<void> {
  ...
  await withConnectedEngine(async (engine) => {
    ...
    if (!apply && result.total_would_apply > 0) {
      console.log(`\nRun \`voltmind schema sync --apply\` to backfill page.type.`);
    }
  });
}
...
async function runAddTypeCmd(args: string[]): Promise<void> {
  ...
  const name = positional[0];
  if (!name) { console.error('Usage: voltmind schema add-type <name> --primitive <p> --prefix <dir/>'); process.exit(2); }
...
async function runRemoveTypeCmd(args: string[]): Promise<void> {
  ...
  if (!name) { console.error('Usage: voltmind schema remove-type <name>'); process.exit(2); }
...
async function runUpdateTypeCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const name = args.filter((a) => !a.startsWith('--'))[0];
  if (!name) { console.error('Usage: voltmind schema update-type <name> [--extractable BOOL] [--expert BOOL] [--primitive P]'); process.exit(2); }
...
async function runAddAliasCmd(args: string[]): Promise<void> {
  ...
  if (pos.length < 2) { console.error('Usage: voltmind schema add-alias <type> <alias>'); process.exit(2); }
...
async function runRemoveAliasCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: voltmind schema remove-alias <type> <alias>'); process.exit(2); }
...
async function runAddPrefixCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: voltmind schema add-prefix <type> <prefix>'); process.exit(2); }
...
async function runRemovePrefixCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: voltmind schema remove-prefix <type> <prefix>'); process.exit(2); }
...
async function runAddLinkTypeCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const name = args.filter((a) => !a.startsWith('--'))[0];
  if (!name) { console.error('Usage: voltmind schema add-link-type <name> [--inverse <verb>] [--page-type <t>] [--target-type <t>]'); process.exit(2); }
...
async function runRemoveLinkTypeCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const name = args.filter((a) => !a.startsWith('--'))[0];
  if (!name) { console.error('Usage: voltmind schema remove-link-type <name>'); process.exit(2); }
...
async function runSetExtractableCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: voltmind schema set-extractable <type> <true|false>'); process.exit(2); }
...
async function runSetExpertRoutingCmd(args: string[]): Promise<void> {
  ...
  const packName = pickPackName({}, args);
  const pos = args.filter((a) => !a.startsWith('--'));
  if (pos.length < 2) { console.error('Usage: voltmind schema set-expert-routing <type> <true|false>'); process.exit(2); }
  const v = parseBool(pos[1]);
  if (v === null) { console.error('Second argument must be true|false'); process.exit(2); }
