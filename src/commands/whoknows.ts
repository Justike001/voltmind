/**
 * voltmind whoknows — "Who should I talk to about X?"
 *
 ...
 *
 * Usage:
 *   voltmind whoknows "lab automation"
 *   voltmind whoknows "fintech compliance" --explain
 *   voltmind whoknows "ai agents" --limit 10 --json
 */
...
const HELP = `Usage: voltmind whoknows <topic> [options]
...
export async function runWhoknows(
  ...
  if ('error' in parsed) {
    console.error(`voltmind whoknows: ${parsed.error}`);
    console.error(HELP);
    process.exit(2);
    return;
  }
