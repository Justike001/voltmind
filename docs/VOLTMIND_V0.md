...
VoltMind is a compiled intelligence system. Not a note-taking app. Not "chat with your notes."
...
There's already a production-grade RAG system (Ruby on Rails, Postgres + pgvector) with 3-tier chunking, hybrid search with RRF, multi-query expansion, and 4-layer dedup. VoltMind ports these proven patterns to a standalone Bun + TypeScript tool.
...
**Library-first distribution.** voltmind is an npm package. OpenClaw installs it as a dependency (`bun add voltmind`), imports the engine directly. Zero-overhead function calls, shared connection pool, TypeScript types. The CLI and MCP server are thin wrappers over the same engine.
...
**Auto-embed during import.** No separate embed step. `voltmind import` chunks and embeds in one pass. Progress bar shows status. `--no-embed` flag for users who want to defer. `embedded_at` column enables `voltmind embed --stale` for backfill.
...
OpenClaw is the AI orchestrator that uses voltmind as its knowledge backend. This is the most common install path.
...
Behind the scenes, `clawhub install voltmind`:
1. Installs the `voltmind` npm package
2. Ships SKILL.md files (ingest, query, maintain, enrich, briefing, migrate)
3. Registers brain tools with the orchestrator
4. Runs `voltmind init --supabase` on first use (guided wizard)
...
`voltmind init --supabase` runs through these steps:
...
Step 3: Config
  ├── Write ~/.voltmind/config.json (0600 permissions)
...
Step 5: First Query
  └── "Try: voltmind query 'what does PG say about doing things that don't scale?'"
```
...
2. **Smart slug resolution.** Fuzzy matching via pg_trgm for reads. Writes require exact slugs. `voltmind get "dont scale"` resolves to `concepts/do-things-that-dont-scale`.
3. **Brain health dashboard.** `voltmind health` shows page count, embed coverage, stale pages, orphans, dead links.
4. **Normalized timeline.** `timeline_entries` table only (no TEXT column). `detail` field supports markdown.
5. **Page version control.** `page_versions` table stores full snapshots (compiled_truth + frontmatter + links + tags). `voltmind history`, `voltmind diff`, `voltmind revert` commands. Revert re-chunks and re-embeds.
6. **Typed links + graph traversal.** `link_type` column (knows, invested_in, works_at, etc.). `voltmind graph` uses recursive CTE with max depth (default 5, configurable via `--depth`).
...
- Supabase service role key in `~/.voltmind/config.json` (0600 permissions)
- MCP stdio transport is inherently local (client spawns `voltmind serve` as subprocess)
- No multi-user, no RLS, no OAuth in v0
- Multi-user path (future): Supabase RLS + per-user API keys

## Upgrade mechanism

`voltmind upgrade` detects the installation method and updates accordingly:

| Path | How |
|------|-----|
| npm | `bun update voltmind` (or npm equivalent) |
...
Budget alternative: `voltmind import --chunker recursive` skips sentence-level embeddings, then `voltmind embed --rechunk --chunker semantic` upgrades later.
...
6. `voltmind init --supabase` works end-to-end
7. `bun test` passes all tests
8. `clawhub install voltmind` installs the skill and runs guided setup
9. `bun add voltmind` + `import { PostgresEngine } from 'voltmind'` works in external project
10. Drift tests pass: CLI and MCP produce identical results
11. `voltmind health` outputs accurate brain health metrics
...
- **`voltmind ask` natural language CLI alias.** Trivial to add. P1 TODO.
