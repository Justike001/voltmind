...
> good thinking survives even when the PR that carried it is closed. voltmind moves
...
> master and (b) genuinely valuable to voltmind users. **Graduating an idea to
...
- **Configurable FTS language** (#580/#581/#582, @rafaelreis-r) — **OPEN, high.**
  ...
  stemmer. A coherent 3-PR set: `VOLTMIND_FTS_LANGUAGE` config → migration recreating
  triggers with the chosen language → `voltmind reindex-search-vector` to change it
...
- **`DATABASE_URL` hijack** (#1884, @awilkinson) — **OPEN, high.** A co-located app's
  ...
  auto-migrates it). Fix precedence: `VOLTMIND_DATABASE_URL` > config.json > `DATABASE_URL`.
...
- **HOME-isolation in tests** (#205/#517/#534 @orendi84, #434 @lloydarmbrust) — **OPEN,
  high.** The E2E suite spawns `voltmind init/import` against the developer's real
  `~/.voltmind/config.json`, clobbering their live DB URL+keys. Isolate HOME to a tmpdir.
...
- **Keyword search ignores page titles** (#1646, @jeades) — **OPEN, high.** `searchKeyword`
  ...
  exact-title `voltmind search` returns nothing while `query` finds it. High-impact, tiny.
...
- **Local-first chat parity** (#1854/#1855/#1858 @starm2010, #1423 @pabloglzg,
  ...
  `json_schema`. The "run voltmind fully local" cluster.
...
- **Dotfile / skills-dir confinement** (#418/#419, @garagon) — **OPEN, high, SECURITY.**
  `.voltmind-source` walk-up trusts any ancestor dotfile (source hijack on shared hosts);
...
- **Destructive reclone gate** (#1705, @mvanhorn) — **OPEN, high, SECURITY.**
  ...
  voltmind-managed, so a re-pointed source can wipe a user's working tree. Gate behind
...
- **Windows / CRLF portability** (#1294 @xwang4-svg, #1149 @samporter-31, #1554 @Sanjays2402,
  ...
  the full story.)*
- **`.voltmindignore` / per-repo exclusion** (#1483 @eepaul; repo-local code filters
  ...
  trees), bloating DB + embedding cost. gitignore-parity `.voltmindignore` + per-source
...
- **MCP tool filtering** (#747, @joelwp) — **OPEN, high.** MCP advertises all ~51 ops to
  every consumer (~10K tokens of schemas, tool confusion); `VOLTMIND_EXPOSED_TOOLS` filters
  the advertised surface.
- **Install-method detection for upgrade** (#538, @brucek) — **OPEN, high.** The README's
  own recommended git-clone+bun-link install detects as `unknown`, so `voltmind upgrade`
...
- **MCP put_page parity + DB→markdown reconciliation** (#438, @rayzhux) — **HELD, high.**
  A frontmatter-only safe auto-link mode for remote callers + `VOLTMIND_BRAIN_ROOT` to render
...
- **Recipe discovery convention** (#1279, @ialmeida-jera) — **OPEN, med.** `~/.voltmind/recipes/`
...
- **Honest stale-lock hint** (#1553, @Sanjays2402) — **OPEN, med.** doctor always says
  `voltmind sync --break-lock`, which silently no-ops on `voltmind-cycle` locks.
