...
- **Pack resolution chain (7 tiers)** — extends model-config's
  ...
  `VOLTMIND_SCHEMA_PACK` env, (3) per-source `--source <id>` override
  via DB config key `schema_pack:source:<id>`, (4) brain-wide DB
  config key `schema_pack`, (5) `voltmind.yml schema:` section,
  (6) `~/.voltmind/config.json schema_pack`, (7) default `voltmind-base`.
...
- Schema packs distribute as `.voltmind-schema` tarballs through the
  v0.37 skillpack pipeline; skillpacks rename to `.voltmind-skillpack`
  for symmetry. Community schema packs propagate the same way
  community skillpacks do.

### Platonic Ideal
A new user clones voltmind and types `voltmind init`. Within 30 seconds
voltmind has read their existing markdown anywhere on disk, proposed a
...
want to. They can publish their pack as a `.voltmind-schema` tarball
for anyone to install and fork.

The 12-month state: `voltmind init` runs `schema detect` automatically,
...
| D3 | `voltmind schema suggest` (LLM-powered) | ~3-5 days | ACCEPTED | Closes the gap from "what exists" to "what your brain implies"; bounded cost via sampling |
...
- **Engine layer:** voltmind-base universal starter pack; 5 composable
...
- **Detect layer:** `voltmind schema detect` SQL-driven heuristic
  clustering proposing a pack manifest matching brain shape.
- **Suggest layer:** `voltmind schema suggest` LLM-powered refinement
  via gateway.chat() over a bounded sample.
- **Inspect layer:** `voltmind schema graph` (ASCII viz),
  `voltmind schema lint` (consistency check), `voltmind schema explain
  <type>` (plain English).
- **Author layer:** `voltmind schema init/use/fork/edit/validate/
...
- **Auto-prompt layer:** TTY-gated interrupt on first unknown-type
  ...
  escape hatch.
- **Distribution layer:** `.voltmind-schema` tarball format; rename
  skillpacks to `.voltmind-skillpack`; v0.37 skillpack pipeline
...
- **voltmind-base:** byte-for-byte reproduces today's hardcoded
...
- **Engine refactor coverage:** the v0.38 plan parameterizes EVERY
  ...
  voltmind-base reproduces today's values for each.
- **Cache + rollback story:**
  ...
  - `eval_candidates` rows (v0.25.0) gain a `schema_pack` column
    so `voltmind eval replay` reproduces the same retrieval space.
  ...
  - Rollback: every `voltmind schema use` operation writes the
    previous pack name to `~/.voltmind/schema-pack-history.jsonl`
    so `voltmind schema use --previous` is one keystroke. Strict-
...
- **Test budget:** ~400-600 cases across unit + e2e per the
  ...
  voltmind-base equivalence regression. **voltmind-base byte-for-byte
  parity is a CI gate**, not a hope — pinned by
  `test/regressions/voltmind-base-equivalence.test.ts` asserting the
...
2. v0.37 skillpack registry currently has zero published packs.
   The `.voltmind-schema` rename and tarball reuse doubles down on a
...
4. `schema suggest` introduces ongoing LLM cost per invocation.
   Bounded by sampling, but sets a precedent for "voltmind commands
   that cost money."
