...
### 10x Check
Instead of "VoltMind has a queue, OpenClaw uses it," make Minions a universal agent
...
protocol. VoltMind IS the agent control plane.

### Platonic Ideal (aspirational North Star, NOT in v1 scope)
Open a terminal, type `voltmind jobs dashboard`. See every agent across every platform.
...
The agent handler does NOT live in VoltMind. VoltMind provides the queue infrastructure
...
**Why this is right:** VoltMind is orchestration, not execution. OpenClaw has the
PI agent core. Hermes has AIAgent. Claude Code has its own loop. Each platform
brings its own engine and registers a handler. VoltMind manages lifecycle, progress,
...
stored as a brain file via `voltmind files upload-raw`.
...
- **Phase 2: Dashboard CLI** — `voltmind jobs dashboard` live TUI showing all agents.
...
1. VoltMind is intentionally evolving from knowledge brain to agent infrastructure (user confirmed)
2. Coupling between OpenClaw and VoltMind's Postgres is acceptable (OpenClaw already depends on VoltMind)
