...
Canonical reference for the JSONL progress stream that `voltmind` writes to
...
- `voltmind doctor` (DB checks, JSONB integrity, markdown body completeness,
  integrity sample)
- `voltmind orphans`
- `voltmind embed`
- `voltmind files sync`
- `voltmind export`
- `voltmind extract [links|timeline|all]` (fs or db source)
- `voltmind import`
- `voltmind sync`
- `voltmind migrate --to …`
...
- `voltmind eval`
- `voltmind apply-migrations` (the orchestrator + every child command)
...
so `voltmind --progress-json doctor` works the same as
`voltmind doctor --progress-json` (the latter also works — per-command
...
When a parent CLI spawns `voltmind …` child processes (mostly in
...
`voltmind jobs work` (the Minion worker daemon) keeps progress in the DB,
...
`get_job_progress` MCP operation or `voltmind jobs get <id>`.
