# Migrating your OpenClaw brain to voltmind v0.41.2.0 (greenfield)
...
through the new ingestion cathedral. Pages land in voltmind with an
...
1. **Upgrade voltmind to v0.41.2.0+:**
   ```bash
   voltmind upgrade
   voltmind --version  # should be 0.41.2.0 or later
   ```

2. **Activate the creator pack** (or voltmind-everything if you also
   want investor + engineer lenses on the same brain):
   ```bash
   voltmind config set schema_pack voltmind-creator
   # OR
   voltmind config set schema_pack voltmind-everything
   ```
...
```bash
voltmind capture --source markdown-greenfield \
  --repo ~/git/brain \
  --dry-run \
  --limit 100
```
...
- `skipped_invalid` — files that failed validation (these append to
  `~/.voltmind/audit/markdown-greenfield-failures-YYYY-Www.jsonl`)

Inspect the audit JSONL:
```bash
ls ~/.voltmind/audit/markdown-greenfield-failures-*.jsonl
cat ~/.voltmind/audit/markdown-greenfield-failures-*.jsonl | jq .
```
...
```bash
voltmind capture --source markdown-greenfield --repo ~/git/brain
```
...
The next `voltmind dream` cycle will:
...
Replace with nothing — voltmind's autopilot already runs extract_atoms +
synthesize_concepts inside every dream cycle when voltmind-creator (or
voltmind-everything) is the active pack.

The OpenClaw skills themselves shrink to thin wrappers:

- `content-atom-extractor` → calls `voltmind dream --phase extract_atoms`
- `concept-synthesis` → calls `voltmind dream --phase synthesize_concepts`
...
voltmind query "imported_from:markdown-greenfield" --type atom --json | \
  jq -r '.[].slug' | xargs -I{} voltmind pages delete {}
# OR hard-delete past the soft-delete window
voltmind pages purge-deleted --older-than 0h
```
