# Give your coding agent a memory: VoltMind + Claude Code / Codex
...
down somewhere. VoltMind is the retrieval layer that fixes that: search, synthesis,
...
- **Path A — I already run a brain** (OpenClaw, Hermes, or any `voltmind serve`
...
Prerequisite for either path: `bun install -g github:garrytan/voltmind`.
...
```bash
voltmind serve --http --bind 0.0.0.0 --public-url https://your-host.example.com
```
...
- **`--bind 0.0.0.0`** — the default bind is `127.0.0.1` (loopback only), which
  ...
  brain" and you didn't pass this, that's why. `voltmind serve --http` warns you at
...
(New brains from `voltmind init` default this ON. Brains upgraded from before
...
```bash
voltmind auth create "laptop-agents"
```

Copy the `voltmind_…` token it prints. It's a long-lived, full-access secret. Treat
...
```bash
# Claude Code
voltmind connect https://your-host.example.com/mcp --token voltmind_xxx --install

# Codex
voltmind connect https://your-host.example.com/mcp --token voltmind_xxx --agent codex --install
```
...
```
Added MCP server 'voltmind' -> https://your-host.example.com/mcp.
...
reads the bearer from `$VOLTMIND_REMOTE_TOKEN` at runtime, so the token never lands
...
```bash
voltmind init --pglite      # 2 seconds; embedded Postgres via WASM, no Docker
```
...
voltmind import ~/notes/

# Or capture as you go (one thought at a time):
voltmind capture "Decided to use PGLite as the default engine: zero-config beats Postgres for <1000 files."
```
...
`voltmind serve` as a stdio subprocess and talks to your local brain directly.
...
(`voltmind query "..."`) and from your agent.
...
any coding agent with VoltMind connected:
...
| Symptom | Cause | Fix |
|---|---|---|
| Agent "can't reach the brain" (Path A) | `voltmind serve --http` bound to loopback | Restart with `--bind 0.0.0.0` |
| `list_skills` returns nothing / errors | Skill publishing OFF on the host | `voltmind config set mcp.publish_skills true` |
| Token rejected on first call | Wrong/expired token | Re-mint with `voltmind auth create`; `--install` smoke-tests it for you |
| `unknown tool: capture` | `capture` is CLI-only, not an MCP tool | Use `put_page` over MCP; `capture` only on the CLI |
| Empty results (Path B) | Brain has nothing in it yet | `voltmind import ~/notes/` or `voltmind capture "..."` |

## Next steps

- Go full autonomous: the overnight enrichment daemon ([dream cycle](../../CHANGELOG.md)) fixes citations, dedupes people, builds scorecards while you sleep. See `voltmind autopilot --install`.
