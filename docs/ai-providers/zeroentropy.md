...
Both land in voltmind v0.35.0.0 behind the openai-compatible recipe path,
...
**Important:** `voltmind config set embedding_model …` is NOT a live
...
from the **file plane** (`~/.voltmind/config.json`) and the **env plane**
(`VOLTMIND_EMBEDDING_MODEL` / `VOLTMIND_EMBEDDING_DIMENSIONS`). The DB plane
is intentionally ignored for these two keys (same posture as today's
Voyage setup).

### Option A — file plane (recommended for stable installs)

Edit `~/.voltmind/config.json`:
...
`voltmind config set embedding_dimensions <one of 2560|1280|640|320|160|80|40>` fix hint.

## Reranker switch — zerank-2

The reranker is the bigger story: voltmind had no cross-encoder reranker
...
before, just with an observable failure surfaced via `voltmind doctor`.
...
`~/.voltmind/audit/rerank-failures-YYYY-Www.jsonl` (ISO-week rotation).
...
| `reranker_health` doctor warns about auth | `ZEROENTROPY_API_KEY` not set or invalid | Re-export the env var; `voltmind models doctor` to verify |
