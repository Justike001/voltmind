...
Users get notified of new VoltMind features conversationally, and the agent walks them through upgrading with post-upgrade migrations that make the new version actually work.

## What the User Gets

Without this: VoltMind ships updates but nobody knows. The user stays on an old
version with stale skills and missing features. Or worse, someone runs
`voltmind upgrade` but skips the post-upgrade steps, leaving new code with old
agent behavior.
...
```
> **VoltMind v0.5.0 is available** (you're on v0.4.0)
>
> What's new:
...
full_upgrade():
  ...
  state = read("~/.voltmind/update-state.json")
...
Prompt: "Run voltmind check-update --json. If update_available is true,
...
Default: daily. Store in agent memory as `voltmind_update_frequency: daily|weekly|off`.
Also persist in `~/.voltmind/update-state.json` so it survives agent context resets.

### Standalone Skillpack Users

If you loaded this SKILLPACK directly (copied or read from GitHub) without
installing voltmind, you can still stay current. Both VOLTMIND_SKILLPACK.md and
...
3. **check-update should run on a daily cron.** Don't rely on the user
   remembering to check for updates. The cron runs `voltmind check-update --json`
...
1. **Run check-update and verify detection.** Execute
   `voltmind check-update --json`. Verify it returns the current version and
