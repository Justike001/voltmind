# `voltmind skillopt` — Self-evolving skills
...
> [Auto-improve a skill with `voltmind skillopt`](../tutorials/improving-skills-with-skillopt.md).
...
voltmind skillopt my-skill --bootstrap-from-skill
...
voltmind skillopt my-skill --bootstrap-reviewed --split 1:1:1
```
...
The audit trail lives at `~/.voltmind/audit/skillopt-YYYY-Www.jsonl`
(ISO-week rotated; honors `VOLTMIND_AUDIT_DIR`).
...
| `--allow-mutate-bundled` | off | Required to mutate voltmind-bundled skills in place — ALSO requires `--held-out` (>=5 rows) or the run hard-refuses |
...
| Bundled-skill gate | D16 | Auto-mutating skills shipped with voltmind (in-place mutation requires `--allow-mutate-bundled` + a `--held-out` set of >=5 benchmark-disjoint tasks; else hard-refuse + proposed.md) |
...
- `voltmind skillify scaffold <name>` — create a new skill (use BEFORE skillopt)
- `voltmind skillpack-check <name>` — audit conformance + skillopt status
- `voltmind check-resolvable` — routing MECE validation (NOT mutated by skillopt)
