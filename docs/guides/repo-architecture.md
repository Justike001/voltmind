...
This is the repo VoltMind indexes.
...
losing your knowledge. You can switch knowledge tools (VoltMind -> something else)
...
**VoltMind indexes the brain repo.** Run `voltmind sync --repo ~/brain/` to keep
the search index current. The agent repo is never indexed by VoltMind.
...
3. **Don't index the agent repo.** VoltMind indexes the brain repo only.
   Running `voltmind sync` against the agent repo pollutes search results
...
3. **Verify VoltMind only indexes brain.** Run `voltmind stats` and check the
