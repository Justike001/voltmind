...
3. **VoltMind search weights compiled truth higher.** `voltmind query` returns compiled
...
4. **The --- separator matters.** VoltMind uses the first standalone `---` after
...
2. **Search for the person.** `voltmind query "Sarah Chen"`. The compiled truth
...
*Part of the [VoltMind Skillpack](../VOLTMIND_SKILLPACK.md). See also: [Source Attribution](source-attribution.md), [Entity Detection](entity-detection.md)*
