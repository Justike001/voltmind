...
VoltMind v0.33 reshapes `voltmind skillpack` from a package manager into a
...
- `voltmind skillpack install <name>` copied bundled skills into your
...
- `voltmind skillpack uninstall` had its own data-loss safeguards (D8
...
voltmind ships a new version, you ask "what changed?" — the agent reads
the diff and decides what (if anything) to integrate.

## The five commands

### `voltmind skillpack scaffold <name> [--workspace PATH]`
...
frontmatter `triggers:` array — voltmind does NOT touch your `RESOLVER.md`
or `AGENTS.md` (see "How agents discover scaffolded skills" below).

```bash
cd ~/git/your-agent-repo
voltmind skillpack scaffold book-mirror
...
### `voltmind skillpack reference <name> [--workspace PATH] [--apply-clean-hunks] [--json]`

Read-only update lens. Diffs voltmind's bundle against your local copy
and emits per-file status (`identical` / `differs` / `missing`) plus
unified diffs for any `differs` entries.

```bash
voltmind skillpack reference book-mirror
# These files live at <voltmind-path> as reference. Read them and
...
parses the diff between voltmind's bundle and your local copy, applies
...
out-of-scope for v0.33), this cannot distinguish "voltmind changed X"
from "you changed X." Applied hunks align everything to voltmind. Use
`--dry-run` first to preview, or run plain `reference` to inspect the
diff before letting auto-apply touch anything.

### `voltmind skillpack migrate-fence [--workspace PATH] [--dry-run]`

One-shot conversion for workspaces on the pre-v0.33 managed-block
model. Strips the `<!-- voltmind:skillpack:begin -->` / `end -->`
...
```bash
cd ~/git/your-agent-repo
voltmind skillpack migrate-fence
...
### `voltmind skillpack scrub-legacy-fence-rows [--workspace PATH] [--dry-run]`
...
### `voltmind skillpack harvest <slug> --from <host-repo-root> [--no-lint] [--dry-run]`

Inverse of scaffold: lifts a proven skill from your host repo back
into voltmind so other clients can scaffold it. Default behavior:

- Symlinks in the host skill dir are rejected (canonical-path
  confinement).
- Privacy linter scans the harvested files against
  `~/.voltmind/harvest-private-patterns.txt` plus built-in defaults
...
This replaces the v0.32 model where `voltmind skillpack install` wrote
...
There's no `voltmind skillpack uninstall` command in v0.33. The files
...
- **New host repo, want a voltmind skill** → `scaffold`
- **voltmind shipped a new version, want to see what's changed**
...
- **Lift your fork's skill back into voltmind** → `harvest` + the
