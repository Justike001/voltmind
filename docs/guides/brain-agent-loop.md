...
4. **External APIs are fallback, not primary.** `voltmind search` before
   Brave Search. `voltmind get` before Crustdata. The brain has relationship
...
4. **Check the sync.** After a conversation, run `voltmind search "{topic}"`
   from the CLI. The new information should be searchable.

---

*Part of the [VoltMind Skillpack](../VOLTMIND_SKILLPACK.md). See also: [Entity Detection](entity-detection.md), [Brain-First Lookup](brain-first-lookup.md)*
