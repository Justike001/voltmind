...
set up (no `~/.voltmind/preferences.json`, autopilot still inline, cron jobs
...
mega-bug (migration fires automatically on `voltmind upgrade` and via
...
but not wired: schema is v7, but no `~/.voltmind/preferences.json`,
...
```
[FAIL] minions_migration: MINIONS HALF-INSTALLED (partial migration: 0.11.0). Run: voltmind apply-migrations --yes
```

or

```
[FAIL] minions_config: MINIONS HALF-INSTALLED (schema v7+ but no ~/.voltmind/preferences.json). Run: voltmind apply-migrations --yes
```

For a machine-readable report (cron-friendly):

```bash
voltmind skillpack-check --quiet && echo healthy || echo needs_action
voltmind skillpack-check | jq -r '.actions[]'    # prints the exact commands to run
```
...
Reads `~/.voltmind/migrations/completed.jsonl`, diffs against the TS
migration registry, runs whatever's pending. Seven phases:

```
A. Schema        voltmind init --migrate-only
B. Smoke         voltmind jobs smoke
C. Mode          prompt (or --yes default pain_triggered)
D. Prefs         write ~/.voltmind/preferences.json
E. Host          AGENTS.md marker injection + cron rewrites for voltmind
                 builtins; JSONL TODOs for host-specific handlers
F. Install       voltmind autopilot --install (env-aware)
G. Record        append completed.jsonl status:"complete"
```

If Phase E emits TODOs for host-specific handlers (e.g. your OpenClaw's
~29 non-voltmind crons), the migration finishes with `status: "partial"`.
...
host repo, then re-runs `voltmind apply-migrations --yes`. Newly
...
```bash
curl -fsSL https://raw.githubusercontent.com/garrytan/voltmind/v0.11.1/scripts/fix-v0.11.0.sh | bash
```

This bash script does what apply-migrations does from a shell environment:

1. `voltmind init --migrate-only` — schema v7.
2. `voltmind jobs smoke` — verify Minions health.
3. Prompt for `minion_mode` (defaults `pain_triggered` on non-TTY).
4. Write `~/.voltmind/preferences.json` atomically.
5. Append `~/.voltmind/migrations/completed.jsonl` with `status: "partial"`
...
7. Print the next step: `Run: voltmind autopilot --install`.

Once v0.11.1 is installed, re-run `voltmind apply-migrations --yes` to
...
cat ~/.voltmind/preferences.json

# 2. Migration recorded
cat ~/.voltmind/migrations/completed.jsonl
...
cat ~/.voltmind/migrations/pending-host-work.jsonl 2>/dev/null || echo "(none — all host work is done)"

# 6. Doctor + skillpack-check should both be clean
voltmind doctor
voltmind skillpack-check --quiet && echo ok
```
...
- **Phase B smoke fails:** the schema didn't apply. Check
  `~/.voltmind/config.json` has a valid `database_url` (or `database_path`
  for PGLite). Run `voltmind init --migrate-only` directly and look at
  the error.
...
- **Pending host work never clears:** your host agent hasn't shipped
  ...
  `~/.voltmind/migrations/pending-host-work.jsonl`, open
