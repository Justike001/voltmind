# How a downstream agent should talk to voltmind

This guide is for authors of downstream agents (hermes, openclaw, future
forks) that need to call voltmind operations from their own runtime. Reading
this first will save you a debugging cycle: voltmind has **two distinct
...
The host runs voltmind as a long-lived HTTP server:

```bash
VOLTMIND_ALLOW_SHELL_JOBS=1 voltmind serve --http --port 3131
```

The agent registers as an OAuth client (one-time):

```bash
voltmind auth register-client hermes \
...
grant. Secrets stay in the voltmind serve process; the agent never sees
DATABASE_URL or API keys.

Thin-client mode (`voltmind init --mcp-only`) gives the agent the same
client-credentials wiring, plus the `voltmind` CLI itself routes MCP-eligible
commands through the configured remote MCP. The agent can call
`voltmind search` / `voltmind query` directly and the CLI does the OAuth dance.
...
- `init` (creates `~/.voltmind/`)
- `secrets` (config management)

For these, the agent cannot route through HTTP MCP. The only path is to run
`voltmind` as a CLI subprocess. The recommended pattern is to submit the
subprocess as a shell job to the voltmind Minions worker so retry / backoff /
...
env as `VOLTMIND_DATABASE_URL`. The DB row in `minion_jobs.data` carries the
...
- Pre-v0.36.5.0 callers passed `env: { VOLTMIND_DATABASE_URL: "postgresql://..." }`
...
voltmind jobs submit shell --params '{...}' --follow

# Persistent worker (Postgres only — PGLite uses --follow inline):
VOLTMIND_ALLOW_SHELL_JOBS=1 voltmind jobs work
```

`VOLTMIND_ALLOW_SHELL_JOBS=1` is the worker-side opt-in. Without it, shell jobs
...
- **Free-form names.** `inherit:` accepts any snake_case config-key on your
  ...
  field you stuff into `~/.voltmind/config.json`. The agent picks what it
  needs.
...
`voltmind config set database_url <value>` or via `VOLTMIND_DATABASE_URL` /
