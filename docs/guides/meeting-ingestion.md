...
on new_meeting_transcript(meeting):
    ...
    for person in meeting.attendees + meeting.mentioned_people:
        voltmind add_timeline_entry <person_slug> \
    ...
    for company in meeting.mentioned_companies:
        voltmind add_timeline_entry <company_slug> \
    ...
    for entity in all_entities_mentioned:
        voltmind add_link <slug> <entity_slug>   # meeting -> entity
        voltmind add_link <entity_slug> <slug>    # entity -> meeting
...
1. After ingesting a meeting, run `voltmind get meetings/{date}-{slug}`. Confirm the page has the agent's analysis above the bar and the full diarized transcript below it.
2. For each attendee, run `voltmind get <attendee_slug>`. Check that their timeline has a new entry referencing the meeting with specific insights (not just "attended meeting").
3. Pick a company mentioned in the meeting. Run `voltmind get <company_slug>`. Confirm a timeline entry exists referencing what was discussed about the company.
4. Run `voltmind get_links meetings/{date}-{slug}`. Verify back-links exist to all attendee and entity pages.
5. Run `voltmind search "{meeting_topic}"`. Confirm the meeting page appears in search results (verifies sync ran).
