...
1. Open any brain page with `voltmind get <slug>`. Read the compiled truth section above the bar. Every factual claim should have an inline `[Source: ...]` citation.
2. Search for tweet references: `voltmind search "X/@"`. Every result should have a full URL, not just an @handle.
