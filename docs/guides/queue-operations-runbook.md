...
- **waiting-depth**: any per-name queue deeper than 10 (override via
  `VOLTMIND_QUEUE_WAITING_THRESHOLD`). Signals a missing `maxWaiting`.
...
- **waiting-depth** — Submitters are piling up jobs faster than workers
  ...
  `VOLTMIND_QUEUE_WAITING_THRESHOLD=50 voltmind doctor`.
...
- B3 — `voltmind doctor --fix` learns to rescue queue wedges.
