# Type Taxonomy (v0.41.22: voltmind-base-v2)

> The 14-canonical-type DRY/MECE taxonomy shipped in v0.41.22. Predecessor
> `voltmind-base` (24 types) stays bundled for back-compat; v0.42+ installs
> default to `voltmind-base-v2`.

## Why

A production voltmind brain (186K pages) had accreted **94 distinct
...
```
voltmind onboard --check                         # surfaces pack_upgrade_available
        ↓
voltmind onboard --check --explain               # per-cluster narrative dry-run
        ↓
voltmind jobs submit unify-types \               # PROTECTED + manual_only
  --allow-protected \
  --params '{"target_pack":"voltmind-base-v2"}'
        ↓
Handler runs 4 phases:
  ┌─────────────────────────────────────┐
  │ Phase 1: Preflight + lock           │ → voltmind-unify db-lock (60min TTL)
  ├─────────────────────────────────────┤
  ...
  └─────────────────────────────────────┘
        ↓
voltmind onboard --check                         # pack_upgrade_available cleared
...
| Page-to-link | Source page soft-deleted with 72h TTL. `voltmind pages restore <slug>` within 72h. Link row stays harmless if source restored. |
| Page-to-alias | Source page soft-deleted with 72h TTL. `voltmind pages restore <slug>` within 72h. Alias row stays harmless (or `DELETE FROM slug_aliases WHERE alias_slug = <slug>` to clean up). |
| Active-pack flip | `voltmind schema use voltmind-base` reverses the flip. |
...
1. Fork voltmind-base-v2: `voltmind schema fork voltmind-base-v2 my-pack`
2. Edit your fork to add page_types + mapping_rules covering your
   custom domain.
3. Target your fork: `voltmind jobs submit unify-types --allow-protected
   --params '{"target_pack":"my-pack"}'`

Your fork can also declare `migration_from: {pack: voltmind-base-v2,
