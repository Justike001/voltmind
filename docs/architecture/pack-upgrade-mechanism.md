...
> How `voltmind-base@1.x → voltmind-base-v2@1.0.0` (and any future pack
...
│  Pack ships bundled OR via ~/.voltmind/schema-packs/<name>/      │
└──────────────────────────┬─────────────────────────────────────┘
                           ↓
┌────────────────────────────────────────────────────────────────┐
...
│         → walks BUNDLED_PACK_NAMES + ~/.voltmind/schema-packs/   │
...
│  USER DECIDES                                                  │
│                                                                │
│  voltmind onboard --check shows finding                          │
│  voltmind onboard --check --explain shows per-cluster narrative  │
│  User reviews; if OK, runs:                                    │
│    voltmind jobs submit unify-types --allow-protected \          │
...
│  • Next voltmind onboard --check shows ok                        │
└────────────────────────────────────────────────────────────────┘
```
...
Walks `BUNDLED_PACK_NAMES` (currently `voltmind-base`,
`voltmind-recommended`, `voltmind-creator`, `voltmind-investor`,
`voltmind-engineer`, `voltmind-everything`, `voltmind-base-v2`). For each
...
packs at `~/.voltmind/schema-packs/*/pack.yaml` (defer to v0.43 since the
...
migration_from:
  pack: voltmind-base-v2
  version: "1.x"

page_types:
  # Inherit voltmind-base-v2's 15 types here (or use extends to merge
...
Drop at `~/.voltmind/schema-packs/voltmind-academic-v1/pack.yaml`.
Discoverable via `voltmind schema list`. Activatable via
`voltmind schema use voltmind-academic-v1`. Once active, the
`pack_upgrade_available` check fires for any brain on
`voltmind-base-v2@1.x` and surfaces a `unify-types` RemediationStep
targeting your pack.

## Lock + concurrency

`voltmind-unify` is a dedicated `voltmind_cycle_locks` row name (60min
TTL). The handler acquires it before any apply phase + releases in
`finally`. Two simultaneous `voltmind jobs submit unify-types`
invocations: second one fails fast at lock acquisition with a clear
error. Same pattern as `voltmind-sync` (v0.22.13 PR #490).

## Audit trail

Every unify run writes to `~/.voltmind/audit/schema-unify-YYYY-Www.jsonl`
...
add `VOLTMIND_AUDIT_FULL=1` (v0.43+ TODO; not yet wired).
...
- Automatic rollback (today: manual SQL or `voltmind pages restore`)
- LLM-assisted mapping_rules codegen from production data (`voltmind
  schema detect-mappings`; deferred to v0.43+)

## Reference

- Pack file: `src/core/schema-pack/base/voltmind-base-v2.yaml`
