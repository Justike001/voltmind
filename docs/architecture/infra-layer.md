...
VoltMind uses Reciprocal Rank Fusion (RRF) to merge vector and keyword search:
...
VoltMind is the deterministic layer. Skills and recipes are the latent space layer.

See [Thin Harness, Fat Skills](../ethos/THIN_HARNESS_FAT_SKILLS.md) for the full
architecture philosophy.

- **VoltMind CLI** = thin harness (same input → same output)
...
The agent reads the skill/recipe and uses VoltMind's deterministic tools to do the work.
