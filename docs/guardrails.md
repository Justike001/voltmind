...
VoltMind exposes **vendor-neutral guardrail seams** at the boundaries where
...
- **Observe-only.** `runGuardrails()` returns `void`. Callers never branch on a
  ...
  block, rewrite, drop, retry, or reorder VoltMind behavior. Enforcement, if ever
...
- **No verdict persistence.** VoltMind writes no guardrail rows. Providers own
...
through `importFromContent` / `importCodeFile`: `voltmind import`, sync, capture,
...
```ts
import { registerGuardrailProvider, type GuardrailInput } from 'voltmind/core/guardrails';
...
registerGuardrailProvider({
  ...
  async classify(input: GuardrailInput) {
    ...
    // VoltMind — return a typed verdict only if your own audit code consumes it.
...
VoltMind deliberately keeps the seam minimal. The provider owns:

- **Timeout discipline.** VoltMind does not impose a timeout in `runGuardrails`
...
Because the verdict is ignored by VoltMind, "shadow mode" requires *no* special
VoltMind flag — it is the only mode this interface supports. Enforcement would be
