...
>
> - **WARN-5 root cause:** `voltmind capture --help` is minimal because
...
|----------|------|------|------|
| `voltmind capture` — basic | 7 | 0 | 0 |
| `voltmind capture` — edge cases | 3 | 3 | 1 |
...
voltmind capture --file /tmp/test.md
voltmind get inbox/2026-05-22-XXXXXX
```
...
```
voltmind capture: put_page failed: insert or update on table "pages" violates
...
Error: source 'my-source' is not registered. Register it first:
  voltmind sources add my-source --path /path/to/source
```
...
connection: connect() has not been called. Fix: Run voltmind init --supabase
```
...
```
$ voltmind capture --help
Usage: voltmind capture

voltmind capture - run voltmind --help for the full command list.
```
...
### WARN-6: `capture`, `brainstorm`, `lsd` missing from `voltmind --help`
...
`voltmind --help` lists every other command (get, put, search, etc.) but
...
voltmind capture --file /tmp/binary.bin
```
...
- **`voltmind capture "text"`** — works perfectly for the simple case.
  Frontmatter stamps, JSON output, disk write, immediate searchability.
- **`voltmind capture --file`** — works for plain text and simple markdown
  (frontmatter doubling bug only affects files with existing frontmatter).
- **`voltmind capture ""`** and no-args — clean error messages:
  `"provide content positionally, --file PATH, or --stdin"`.
- **`voltmind capture --file /nonexistent`** — clean ENOENT error.
- **Unicode/emoji** — `voltmind capture "测试 🧠🔥 émojis"` works perfectly.
...
- **X-Voltmind-Source-Id header** — custom source ID flows through correctly.
- Legacy `X-VoltMind-*` ingest headers remain accepted as compatibility aliases
  and return `Deprecation: true` plus a migration `Warning` response header.
...
- **Soft delete + recovery** — cleanup via `voltmind delete` works with
  72h recovery window.
...
2. **Main help completeness** — add `capture`, `brainstorm`, and `lsd` to
   the `voltmind --help` command listing. These are user-facing features that
   can't be discovered.
