...
When building VoltMind features, use this decision guide:
...
**VoltMind examples:**
- `voltmind integrations list` = **Code** (reads files, checks env vars, deterministic)
- `voltmind integrations status` = **Code** (checks env vars + heartbeat, deterministic)
- `voltmind integrations doctor` = **Code** (runs health checks, deterministic)
- `voltmind integrations stats` = **Code** (aggregates JSONL, deterministic)
