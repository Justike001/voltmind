...
You'll fork the bundled `voltmind-base` pack, add a custom `researcher` page type, import a handful of placeholder researcher pages, backfill their `page.type` column with one command, then prove the wiring works by running `voltmind whoknows` and seeing your new type surface in results. End state: a forked-and-active pack on disk, ~5 pages typed as `researcher`, and a query that proves the pack-aware routing fires end-to-end.
...
- voltmind v0.40.7.0 or later (`voltmind --version` to check)
- A brain that's been initialized (`voltmind init` already run; either PGLite or Postgres is fine)
...
Output: `Forked 'voltmind-base' → 'mine' at ~/.voltmind/schema-packs/mine/pack.json`.

The fork is a byte-for-byte copy of `voltmind-base` living at `~/.voltmind/schema-packs/mine/pack.json`. Now you have a writable pack you can mutate.
...
Run `voltmind schema active --json` again to confirm `pack_name` is now `mine` and `source_tier` is `home-config` (read from `~/.voltmind/config.json`).
...
```bash
voltmind schema add-type researcher \
  --primitive entity \
...
- An audit row landed in `~/.voltmind/audit/schema-mutations-YYYY-Www.jsonl` with your type name SHA-8-redacted and the prefix's first segment only (`people`) for privacy.

Verify the type is in the pack:

```bash
voltmind schema explain researcher
```
...
```bash
voltmind schema stats --json | jq '.aggregate, .dead_prefixes'
```
...
```bash
voltmind whoknows "machine learning"
```
...
You now have:
- A fork of `voltmind-base` named `mine` at `~/.voltmind/schema-packs/mine/pack.json`, active in your brain via `~/.voltmind/config.json`.
...
- 3 pages typed as `researcher` (backfilled from disk via `voltmind schema sync --apply`).
- A query path that routes through the new type: `voltmind whoknows` reads the pack and includes `researcher` in its type filter.
...
voltmind schema graph
```

The graph now shows `researcher --(authored)--> paper`.

**Add aliases for query closure.** If you want `voltmind query researcher` to also surface `person` rows (because researchers ARE people):

```bash
voltmind schema add-alias researcher person
```
...
**Commit your pack to source control.** If `~/.voltmind/schema-packs/mine/` is a git repo, commit `pack.json` and push. Your pack survives across machines, and the `mutation_count_anomaly` lint rule will nudge you when you hit >50 mutations in a week (the "you should be committing this" signal).
...
**Undo a mistake.** Every mutation primitive has an inverse (`remove-type`, `remove-alias`, `remove-prefix`, `remove-link-type`, `set-extractable false`, etc.). If you fork twice and want to revert, `voltmind schema downgrade` restores the previous active pack from `~/.voltmind/schema-pack-history.jsonl`.

## Related docs

- **Reference:** `voltmind schema --help` for the full 22-verb CLI surface; CLAUDE.md's "Schema Cathedral v3 (v0.40.7.0)" section for the module-by-module architecture.
