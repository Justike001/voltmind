...
VoltMind's embedding-spend gates in one place: every gate, its config key, default,
whether it blocks or just informs, how to widen or disable it, and how the
`spend.posture` switch governs all of them.

The orienting idea: **VoltMind itself is rounding error; the spend that matters is
...
```bash
voltmind config set spend.posture tokenmax   # all cost gates become informational
voltmind config set spend.posture gated      # default — gates enforce
```
...
- **Non-interactive (cron/agent):** **auto-defers** embeds to capped backfill jobs and
  ...
  `voltmind embed --stale`. Pass `--yes` to embed inline instead.
...
- **Single-source `voltmind sync`** carries the same gate as `sync --all` (it previously
...
voltmind config set spend.posture tokenmax

# Widen the sync inline floor to $5:
voltmind config set sync.cost_gate_min_usd 5

# Disable the sync inline floor entirely:
voltmind config set sync.cost_gate_min_usd off

# Lift the backfill 24h spend cap:
voltmind config set embed.backfill_max_usd_per_source_24h off

# Run enrich uncapped non-interactively:
voltmind enrich --max-usd off        # or: voltmind config set spend.posture tokenmax
```
