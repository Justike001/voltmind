# Releasing & contributing (voltmind)
...
- `bun run ci:local` runs the entire stack inside Docker: gitleaks (host),
  ...
  the host port with `VOLTMIND_CI_PG_PORT=5435 bun run ci:local` if 5434 collides.
...
- Upgrade instructions: `voltmind upgrade` + any manual step if needed.
...
`voltmind brainstorm` does and how to use it. If a fact only exists because
...
**Smell test:** read the entry as a stranger who has never touched voltmind.
...
must be readable by someone who does NOT know voltmind's internals. No file paths,
...
**The smell test:** if someone who has never opened voltmind reads the first 150
...
- Latest `voltmind-evals/docs/benchmarks/[latest].md` for headline numbers (sibling repo)
...
Why: `voltmind upgrade` runs `voltmind post-upgrade` which runs `voltmind apply-migrations`.
...
recovery path; the v0.13+ `~/.voltmind/upgrade-errors.jsonl` trail + `voltmind doctor`
...
`voltmind upgrade` should do this automatically. If it didn't, or if `voltmind doctor`
...
3. **Verify the outcome:**
   ```bash
   [release-specific verify commands, e.g. `voltmind graph ... --depth 2`]
   voltmind stats
   ```
4. **If any step fails or the numbers look wrong,** please file an issue:
   https://github.com/garrytan/voltmind/issues with:
   - output of `voltmind doctor`
   - contents of `~/.voltmind/upgrade-errors.jsonl` if it exists
   - which step broke

   This feedback loop is how the voltmind maintainers find fragile upgrade paths. Thank you.
```
...
- Good: "Your agent now verifies the entire VoltMind installation end-to-end, catching
...
- Schema changes that require `voltmind init` or manual SQL
...
VoltMind's job is to deliver a canonical, working setup to every user on upgrade.
Anything that looks like a "host-repo change" — AGENTS.md, cron manifests,
launchctl units, config files outside `~/.voltmind/` — is a VoltMind migration
...
`~/.voltmind/migrations/pending-host-work.jsonl` + the host agent walks the
...
`~/.voltmind/update-state.json` tracks which recommended schema directories the user
...
2. `git push origin HEAD:<branch-name>` — push the same branch to the base
   repo (origin points at `garrytan/voltmind`, not the fork). This is the move
