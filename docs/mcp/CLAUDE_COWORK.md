# Connect VoltMind to Claude Cowork

Two ways to get VoltMind into Cowork sessions:
...
3. Add Bearer token authentication in Advanced Settings
   (create one with `voltmind auth create "cowork"`)
4. Save
...
If you already have VoltMind configured in Claude Desktop (via `voltmind serve`
...
This means: if `voltmind serve` is running and configured in Claude Desktop,
...
- **Local Bridge:** zero extra setup if Claude Desktop already has VoltMind, but requires your machine to be running
