...
// These tests exercise the filesystem walker that feeds `voltmind import`.
...
describe('collectMarkdownFiles — symlink containment', () => {
  ...
  beforeEach(() => {
    ...
    root = mkdtempSync(join(tmpdir(), 'voltmind-walker-root-'));
    secretDir = mkdtempSync(join(tmpdir(), 'voltmind-walker-secret-'));
  });
