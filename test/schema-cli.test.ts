// v0.38 Phase C: voltmind schema CLI smoke tests.
//
...
function voltmind(
  ...
  // any subprocess test that needs VOLTMIND_HOME isolation.
...
describe('voltmind schema CLI (Phase C)', () => {
  test('schema with no subcommand shows help text', () => {
    // Note: `schema --help` is intercepted by the CLI's parent help system
    // and prints generic help (`voltmind --help` for full command list). The
    // schema-specific help fires when no subcommand is provided.
    const r = voltmind(['schema']);
    expect(r.stdout + r.stderr).toMatch(/schema|active|list|show|validate|use/i);
  });

  test('schema list shows voltmind-base bundled', () => {
    const r = voltmind(['schema', 'list']);
    expect(r.code).toBe(0);
    expect(r.stdout).toContain('Bundled packs:');
    expect(r.stdout).toContain('voltmind-base');
  });

  test('schema show voltmind-base prints manifest details', () => {
    const r = voltmind(['schema', 'show', 'voltmind-base']);
    expect(r.code).toBe(0);
    expect(r.stdout).toContain('voltmind-base v1.0.0');
    // v0.41.11.0: page types extended from 22 to 24 by promoting
    // `conversation` and `atom` into voltmind-base.
  ...
  test('schema validate voltmind-base passes', () => {
    const r = voltmind(['schema', 'validate', 'voltmind-base']);
    expect(r.code).toBe(0);
  ...
  test('schema active reports default resolution', () => {
    const r = voltmind(['schema', 'active']);
    expect(r.code).toBe(0);
  ...
  test('schema show unknown-pack errors with hint', () => {
    const r = voltmind(['schema', 'show', 'nonexistent-pack']);
  ...
  test('unknown subcommand exits with hint', () => {
    const r = voltmind(['schema', 'frobnicate']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('Unknown schema subcommand');
  });

  test('schema use without arg shows usage hint', () => {
    const r = voltmind(['schema', 'use']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('Usage:');
  });
});

describe('voltmind schema use (Phase C, gap-fill T3)', () => {
  let home: string;

  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'voltmind-schema-use-'));
  });
  ...
  test('writes schema_pack to ~/.voltmind/config.json on happy path', () => {
    const r = voltmind(['schema', 'use', 'voltmind-base'], { VOLTMIND_HOME: home });
    expect(r.code).toBe(0);
    expect(r.stdout).toContain('Active schema pack set to: voltmind-base');
    expect(r.stdout).toContain('schema active');
    const cfgPath = join(home, '.voltmind', 'config.json');
    expect(existsSync(cfgPath)).toBe(true);
    const cfg = JSON.parse(readFileSync(cfgPath, 'utf-8'));
    expect(cfg.schema_pack).toBe('voltmind-base');
  });
  ...
  test('unknown pack rejected with exit 1 + paste-ready hint', () => {
    const r = voltmind(['schema', 'use', 'no-such-pack-xyz'], { VOLTMIND_HOME: home });
