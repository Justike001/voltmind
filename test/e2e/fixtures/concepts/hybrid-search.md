...
deliver results that are both semantically relevant and keyword-precise. VoltMind uses
...
4. **Multi-query expansion** — VoltMind generates multiple search queries from a single
...
## Implementation in VoltMind

VoltMind implements hybrid search in `src/core/search/` using Postgres as the single
...
to ship hybrid search with RRF in VoltMind v0.3 as the highest priority feature.

### 2025-04-01 — Shipped in v0.3

Hybrid search shipped as part of VoltMind v0.3. Initial results show significant
