...
beforeEach(async () => {
  ...
  const eng = getEngine();
  try { await (eng as any).sql`DELETE FROM gbrain_cycle_locks WHERE id LIKE 'voltmind-sync:%'`; } catch { /* */ }
});
...
describeE2E('v0.41.6.0 — sync lock recovery scenarios', () => {
  ...
  test('--break-lock with alive local PID refuses with --force-break-lock hint', async () => {
    ...
    // Cleanup.
    await (eng as any).sql`DELETE FROM gbrain_cycle_locks WHERE id = 'voltmind-sync:default'`;
  });
