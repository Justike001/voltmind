...
beforeAll(async () => {
  if (skip) return;
  tmpHome = mkdtempSync(join(tmpdir(), 'voltmind-lock-recovery-e2e-'));
  await setupDB();
});
...
beforeEach(async () => {
  ...
  repoDir = mkdtempSync(join(tmpdir(), 'voltmind-lock-recovery-repo-'));
  ...
  const eng = getEngine();
  try { await (eng as any).sql`DELETE FROM voltmind_cycle_locks WHERE id LIKE 'voltmind-sync:%'`; } catch { /* */ }
});
...
describeE2E('v0.41.6.0 — sync lock recovery scenarios', () => {
  ...
  test('lock-busy error message includes PID + hostname + age + --break-lock hint', async () => {
    ...
    const eng = getEngine();
    const lockKey = 'voltmind-sync:default';
  ...
  test('--break-lock with TTL-expired row clears the lock', async () => {
    ...
    await (eng as any).sql`
      ...
      VALUES ('voltmind-sync:default', 99999, ${hostname()}, NOW() - INTERVAL '1 hour', NOW() - INTERVAL '30 minutes')
    `;
    ...
    const snap = await inspectLock(eng, 'voltmind-sync:default');
  ...
  test('--break-lock with alive local PID refuses with --force-break-lock hint', async () => {
    ...
    await (eng as any).sql`
      INSERT INTO voltmind_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
      VALUES ('voltmind-sync:default', ${process.pid}, ${hostname()}, NOW(), NOW() + INTERVAL '30 minutes')
    `;
    ...
    // Cleanup.
    await (eng as any).sql`DELETE FROM voltmind_cycle_locks WHERE id = 'voltmind-sync:default'`;
  });
  ...
  test.skip('pipe through `head -5` exits cleanly, next sync runs without lock-busy', async () => {
    // Run `voltmind sync ... | head -5` via shell.
