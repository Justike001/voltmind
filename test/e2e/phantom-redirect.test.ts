...
beforeEach(async () => {
  ...
  // v0.40: phantom redirect now uses per-source lock id (voltmind-sync:<source>).
  await engine.executeRaw(`DELETE FROM voltmind_cycle_locks WHERE id='voltmind-sync:default'`);
});
...
describeMaybe('phantom-redirect E2E (Postgres)', () => {
  test('bulk-pile: 60 phantoms with cap=50 → 50 redirected, more_pending=true', async () => {
    ...
    try {
      ...
      const engine = getEngine();
      await withEnv({ VOLTMIND_PHANTOM_REDIRECT_LIMIT: '50' }, async () => {
  ...
  test('concurrent-sync race: external lock holder → pass skipped, audit entry', async () => {
    ...
    try {
      ...
      const engine = getEngine();
      // Take the voltmind-sync lock manually (simulates concurrent voltmind sync).
      ...
      // v0.40 D16: phantom acquires per-source lock; default source = voltmind-sync:default.
      await engine.executeRaw(
        `INSERT INTO voltmind_cycle_locks (id, holder_pid, holder_host, acquired_at, ttl_expires_at)
         VALUES ('voltmind-sync:default', 9999, 'simulated-other-host', now(), now() + interval '1 hour')
