 ...
 * Covers what the unit tests can't: the voltmind_cycle_locks table's
...
describeE2E('E2E: runCycle against real Postgres', () => {
  ...
  test('latest migrations expose the voltmind_cycle_locks table', async () => {
    const conn = getConn();
    const rows = await conn.unsafe(
      `SELECT tablename FROM pg_tables WHERE tablename = 'voltmind_cycle_locks'`,
    );
  ...
  test('dry-run full cycle: zero DB writes + zero filesystem changes', async () => {
    ...
    expect(report.phases.length).toBe(21); // v0.41 lens phases + conversation backfill + tracking maintenance
    ...
    const locks = await conn.unsafe(`SELECT COUNT(*)::int AS n FROM voltmind_cycle_locks`);
  ...
  test('concurrent cycle is blocked by the lock (status:skipped)', async () => {
    ...
    try {
      ...
      await conn.unsafe(`DELETE FROM voltmind_cycle_locks WHERE id = 'voltmind-cycle'`);
    }
  });
