...
//
// Pins the contract that extractableTypesFromPack(voltmind-base) returns
// the configured eligible set declared by voltmind-base.yaml. The pre-v0.38
...
// `extractable: true` in voltmind-base.yaml.
const V0_41_11_ADDED_ELIGIBLE = ['concept', 'conversation'];
const CURRENT_ELIGIBLE = [...LEGACY_ELIGIBLE, ...V0_41_11_ADDED_ELIGIBLE];

describe('extractableTypesFromPack (T7d) — voltmind-base parity', () => {
  test('voltmind-base extractable set matches the configured eligible list', () => {
    ...
    // Entity-shape + annotation-shape types stay non-extractable in voltmind-base.
