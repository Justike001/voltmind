...
You inherit a voltmind v0.18 brain (the harness has already replayed a seed SQL dump into a PGLite database). Walk through the upgrade path:

1. **Run `voltmind doctor --json`** first. Note any warnings or fix-hints.
2. **Run `voltmind init --pglite`** with the existing database path. The migration chain should detect the old `schema_version` and walk forward to the latest.
3. **Run `voltmind doctor --json` again.** The `status` field should be `"ok"`.
4. **Verify queries still work:** `voltmind query "alice"` should return results from the seeded brain.
...
- `voltmind init --pglite` didn't recognize the existing brain
