/**
 * v0.40.6.1 — voltmind models doctor reranker probe divergence fix.
 *
 * Pre-v0.40.6.1 the reranker probe read `getRerankerModel()` from the
 * gateway, which is fed from `VoltMindConfig.reranker_model` — a file-plane
...
describe('resolveLiveRerankerModel — divergence fix', () => {
  ...
  test('returns the mode-bundle default when no override is set', async () => {
    // balanced mode bundle has reranker_enabled: true + reranker_model:
    // the canonical Qwen3-VL reranker baked in. Pre-fix this case returned
