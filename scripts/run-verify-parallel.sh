...
#
# Env overrides:
#   VOLTMIND_VERIFY_TIMEOUT       per-check wallclock cap, seconds (default 120)
#   VOLTMIND_VERIFY_LOG_DIR       where to write per-check logs (default tempdir)
#
# Exit codes:
#   0   all checks passed
...
TIMEOUT="${VOLTMIND_VERIFY_TIMEOUT:-120}"
...
if [ -n "${VOLTMIND_VERIFY_LOG_DIR:-}" ]; then
  LOG_DIR="$VOLTMIND_VERIFY_LOG_DIR"
  mkdir -p "$LOG_DIR" || { echo "ERROR: cannot create $LOG_DIR" >&2; exit 2; }
else
  LOG_DIR="$(mktemp -d /tmp/voltmind-verify-XXXXXX)"
