...
**Cost anchors (downstream agent input cost — voltmind itself is rounding error).**
...
picker copy in `voltmind init` carries the same matrix verbatim** — update
...
`[CDX-23]`. The user's personal `~/.voltmind` brain is NEVER touched —
...
grep -E '(fail\)|✗|error:' /tmp/voltmind_test_output.txt | head -30
```
...
| `VOLTMIND_SYNC_CHECKPOINT_SECONDS` | 10 | Also flush every N seconds (whichever comes first) — bounds worst-case loss regardless of throughput. Flush also fires after the first file. |
...
| `VOLTMIND_SYNC_YIELD_EVERY` | 64 | Yield the event loop (`setTimeout(0)`, NOT `setImmediate` — Bun starves the timers phase under a tight setImmediate loop) every N files so the lock-refresh `setInterval` heartbeat fires mid-import. |
...
of the search-mode pattern but with **env ABOVE config** (incident escape hatch):
    ...
    per-call flag → VOLTMIND_PACE_* env → config (pace.*) → PACE_BUNDLES[mode] → off
